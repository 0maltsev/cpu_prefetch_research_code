#!/usr/bin/env python3
"""Fail-closed Stage 17 operational CLI with external-journal preflight successor.
...
import stage17_state_journal_v11 as journal_runtime
...
def author_ext001(
    *, repository_root: pathlib.Path, evidence_root: pathlib.Path,
    ...
    contract = {
        "schema_version":
            "cpu-prefetch-stage17-read-only-preflight-supporting-contract/9",
    ...
    contract_path = destination / "supporting-contract-v9.json"
    ...
    authorization = {
        "schema_version":
            "cpu-prefetch-stage17-read-only-preflight-authorization/9",
        "authorization_id": authorization_id, "attempt_id": attempt_id,
    ...
    authorization_path = destination / "authorization-v9.json"
    write_external_exclusive(authorization_path, canonical(authorization))
    envelope = {
        "schema_version": "cpu-prefetch-stage17-operational-evidence-envelope/10",
        ...
        "predecessor": {
            ...
            "semantic_policy_v9_sha256":
                policy["predecessor"]["policy_v9"]["sha256"],
        ...
        "authorization": _repository_binding(
            ...
            schema_identity="cpu-prefetch-stage17-read-only-preflight-authorization/9",
        ),
    ...
    envelope_path = destination / "envelope-v10.json"
    validate_schema(
        repository_root,
