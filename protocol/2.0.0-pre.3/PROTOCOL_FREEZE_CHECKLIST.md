...
Protocol version: **`2.0.0-pre.3`**. The paper and implementation specification are an implementation-ready structural protocol, but they are not pilot- or execution-ready. Version `2.0.0-pre.3` adds an incompatible closed-manifest representation for simultaneous blockers without changing observations, gates, estimands, or replacement semantics; all prior lineage and hashes remain preserved. Confirmatory execution is prohibited until every applicable mandatory item is frozen in a dated, treatment-blind, append-only record. `FIXED` means the semantics are normative; it does not imply that a platform mapping or numerical output already exists.

| Item | Required record | Status | Gate while unresolved |
|---|---|---|---|
| Protocol version | semantic version and amendment lineage | **FIXED:** `2.0.0-pre.3` | new sealed graphs must name it; prior instances remain immutable and require explicit versioned conversion |
...
| Simultaneous confirmatory blockers | exhaustive unique blocker set, canonical token ordering, matching singleton or `BLOCKED_MULTIPLE` summary, no retry/replacement authority | **FIXED** | final disposition requires all applicable gate evidence and semantic validation |
| Warm-up seed namespace | disjoint namespace and generator mapping | **OPEN BEFORE PILOT** | no reproducible warm-up |
| Warm-up duration | exactly `5 s` for marked `PRE_FREEZE_BOOTSTRAP` calibration/pilot runs; later `ceil_tick(max(5 s, 10*tau_corr))`, `tau_corr=h_max/rho_min`, with per-run horizons/rates and complete pilot lineage | **FIXED BOOTSTRAP AND FREEZE RULE; OUTPUT BLOCKED** | no calibration/pilot action without the correct phase rule; no confirmatory run until the complete warm-up freeze passes |
...
| Manifest/schema versions | Draft 2020-12 schema IDs, compatibility matrix, positive/negative conformance suite, semantic-validator contract | **FIXED STRUCTURE:** handoff schemas at `2.0.0-pre.3` | implementation must reject mixed graphs and unknown versions; conversion creates a new derived artifact |
