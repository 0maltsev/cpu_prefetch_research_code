#!/usr/bin/env python3
"""Reproducibly render or verify Stage 17 production policy v21.

Adds phase-controller v9's restored `_runtime_from_context` (ADR-0127,
`PROPOSED`/owner-review pending) to the runtime closure, so `author-request`
for Q15-R/Q15-W/Q16a/Q16b/Q16c/the blinded pilot no longer raises
`AttributeError`. Policy v20 and its entire bindings/runtime_closure remain
...
import author_stage17_policy_v20 as predecessor
import stage17_semantic_verifier_v21 as verifier
...
SUCCESSOR_BINDINGS = (
    "docs/decisions/0127-stage17-phase-controller-runtime-context-restoration.md",
    ...
    "tools/stage17_semantic_verifier_v21.py",
...
def render(root: pathlib.Path) -> dict[str, object]:
    ...
    policy.update({
        "schema_version":
            "cpu-prefetch-stage17-operational-evidence-admission-policy/21",
        "policy_id": "STAGE17-OPERATIONAL-EVIDENCE-ADMISSION-POLICY-v21",
        "policy_version": "21",
        ...
        "predecessor_status":
            "SUPERSEDED_ADDS_D127_PHASE_CONTROLLER_RUNTIME_CONTEXT_RESTORATION",
...
def main() -> int:
    ...
    if arguments.check:
        if not destination.is_file() or destination.read_bytes() != payload:
            print("stage17-policy-v21: FAIL: generated policy bytes drifted",
                  file=sys.stderr)
            return 1
        print("stage17-policy-v21: PASS")
        return 0
    if arguments.write:
        ...
        try:
            ...
            os.close(directory)
        print(f"stage17-policy-v21: PASS written={destination}")
        return 0
