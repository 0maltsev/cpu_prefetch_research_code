#!/usr/bin/env python3
"""Fail-closed Stage 17 operational CLI with runtime-cardinality preflight successor.
...
import stage17_phase_controller_v6 as controller
...
def author_ext001(
    *, repository_root: pathlib.Path, evidence_root: pathlib.Path,
    ...
    pre_marker_blocker: pathlib.Path,
    ...
    contract = {
        "schema_version":
            "cpu-prefetch-stage17-read-only-preflight-supporting-contract/10",
    ...
    contract_path = destination / "supporting-contract-v10.json"
    ...
    authorization_path = destination / "authorization-v10.json"
    write_external_exclusive(authorization_path, canonical(authorization))
    envelope = {
        "schema_version": "cpu-prefetch-stage17-operational-evidence-envelope/12",
        ...
        "predecessor": {
            ...
            "semantic_policy_v11_sha256":
                policy["predecessor"]["policy_v11"]["sha256"],
        ...
        "authorization": _repository_binding(
            ...
            schema_identity="cpu-prefetch-stage17-read-only-preflight-authorization/10",
        ),
    ...
    envelope_path = destination / "envelope-v12.json"
    validate_schema(
        repository_root,
        "config/schemas/stage17-read-only-preflight-supporting-contract-v10.schema.json",
        contract,
    )
    validate_schema(
        repository_root,
        "config/schemas/stage17-read-only-preflight-authorization-v10.schema.json",
        authorization,
    )
    validate_schema(
        repository_root,
        "config/schemas/stage17-operational-evidence-envelope-v12.schema.json",
        envelope,
    )
    ...
    try:
        ...
        if arguments.command == "init":
            ...
            result = author_ext001(
                ...
                pre_marker_blocker=arguments.pre_marker_blocker.resolve(),
