#!/usr/bin/env python3
"""Versioned Stage 17 journal runtime for semantic-admission policy v8."""
...
from stage17_semantic_verifier_v8 import (
    evaluate_s17_ext_001_action_readiness,
    verify_policy_v8,
    verify_s17_ext_001_semantics_v8,
)
...
SEMANTIC_POLICY_PATH = pathlib.PurePosixPath(
    "config/stage17/stage17-operational-evidence-admission-policy-v8.json"
)
SEMANTIC_POLICY_SCHEMA_PATH = pathlib.PurePosixPath(
    "config/schemas/stage17-operational-evidence-admission-policy-v8.schema.json"
)
SEMANTIC_ENVELOPE_SCHEMA_PATH = pathlib.PurePosixPath(
    "config/schemas/stage17-operational-evidence-envelope-v8.schema.json"
)
...
def load_semantic_policy(
    ...
    *,
    ...
    validate_schema(policy, root / SEMANTIC_POLICY_SCHEMA_PATH, "semantic-admission policy v8")
    try:
        verify_policy_v8(
            root=root,
            policy=policy,
...
SEMANTIC_VERIFIERS: dict[tuple[str, str, str], SemanticVerifier] = {
    ("S17-EXT-001", "STAGE17-S17-EXT-001-SEMANTIC-VERIFIER", "8"):
        verify_s17_ext_001_semantics_v8,
