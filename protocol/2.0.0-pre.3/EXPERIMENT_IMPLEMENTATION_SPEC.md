...
Protocol version: **`2.0.0-pre.3`**. This version adds the exhaustive simultaneous-blocker manifest representation authorized by Q10/Q11. It preserves the observation model and scientific gates; lineage and prior hashes are preserved in `handoff/PROTOCOL_VERSION.md` and `handoff/AMENDMENTS.md`.
...
pilot, confirmatory roles, diagnostics, and extensions; warm-up observations
never enter inference. It uses the same queue package, software-prefetch
policy, hardware-prefetch state, placement, persistent arena mappings, and
consumer action as the immediately following measured run.

Before a valid warm-up freeze exists, calibration and pilot runs use exactly
five seconds of warm-up. Such runs are explicitly marked
`PRE_FREEZE_BOOTSTRAP`; they are non-confirmatory and cannot enter
confirmatory inference. They retain every ordinary warm-up namespace,
treatment-equality, drain, logical-reset, mapping-preservation, and
start-barrier obligation.

For every valid required pilot run, compute the tail-indicator correlation
horizon independently using Section 10.4; runs are never concatenated. Let
`h_max` be the maximum valid run-level horizon across the complete required
pilot matrix and all prospectively frozen repetitions. For each same eligible
run, define the accepted-event rate as the exact rational accepted count
divided by the measurement-horizon duration; drain time is excluded. Let
`rho_min` be the minimum of those run-level rates. A correctly reconciled
`FULL` run remains valid and contributes its actual accepted count while
failing zero loss separately.

Missing or invalid required evidence, an incomplete matrix, no valid
correlation horizon, or zero accepted count blocks the warm-up freeze. There
is no imputation, favorable filtering, cell-specific duration, implicit retry,
or extension. Define `tau_corr = h_max / rho_min`; its dimension is time. The
frozen later warm-up is
`ceil_tick(max(5 seconds, 10 * tau_corr))`, rounded upward to the accepted
schedule integer time unit. It is fixed before confirmatory execution and is
never revised from confirmatory behavior. The freeze record names every source
run and raw/join/integrity hash, estimator/version, exact rate
numerator/denominator, selected extrema, rounding unit, resulting duration,
validity decision, protocol version, and implementation release.
