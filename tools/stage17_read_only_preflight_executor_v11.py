#!/usr/bin/env python3
"""Stage 17 terminal-compatibility read-only preflight executor successor.
...
import stage17_read_only_preflight_semantic_verifier_v13 as current_semantic
import stage17_state_journal
import stage17_state_journal_v10
import stage17_state_journal_v11
import stage17_state_journal_v15 as current_journal


EXECUTOR_ID = "STAGE17-READ-ONLY-PREFLIGHT-EXECUTOR-v11"
...
def runtime_identity_paths() -> dict[str, str]:
    """Return the exact loaded preflight-policy-v13 runtime closure."""
...
def _all_one_shot_names(context: dict[str, Any]) -> list[str]:
    ...
    names.extend(
        ...
        for suffix in (
            ...
            ".receipt-v5.json", ".receipt-v6.json",
        )
    )
    return names
...
def _write_failure(
    *, prepared: PreparedAction, failure: OperationalFailure, completed: list[str],
    ...
    document = {
        "schema_version": "cpu-prefetch-stage17-read-only-preflight-failure/7",
...
def _write_failure_retention(
    *, prepared: PreparedAction, failure: OperationalFailure,
    ...
    context = prepared.context
    document = {
        "schema_version": "cpu-prefetch-stage17-read-only-preflight-failure-retention/2",
        "attempt_id": context["attempt_id"],
        "attempt_marker_sha256": prepared.attempt_marker_sha256,
        "authorization_id": context["authorization_id"],
        "authorization_sha256": context["authorization_sha256"],
...
def _prospective_validation(
    *, repository_root: pathlib.Path, latest_journal: pathlib.Path,
    ...
    if (
        ...
        action_context = current_semantic.evaluate_s17_ext_001_action_readiness_v13(
...
def execute_once(
    *, repository_root: pathlib.Path, latest_journal: pathlib.Path,
    ...
    try:
        ...
        marker = {
            "schema_version": "cpu-prefetch-stage17-read-only-preflight-attempt/9",
        ...
        try:
            for descriptor, program in zip(prepared.program_descriptors, prepared.programs, strict=True):
                ...
                receipt = {
                    "schema_version": "cpu-prefetch-stage17-read-only-preflight-observation-receipt/6",
                ...
                try:
                    ...
                    receipt_hashes.append(_write_exclusive_at(
                        prepared.directory_fd, f"s17-ro-{ordinal:03d}.receipt-v6.json",
                ...
                _remaining_seconds(deadline_ns)
            completion = {
                "schema_version": "cpu-prefetch-stage17-read-only-preflight-completion/6",
...
def main() -> int:
    ...
    try:
        ...
        print(f"stage17-read-only-preflight-v11: FAIL: {exception}", file=sys.stderr)
        return 1
    print("stage17-read-only-preflight-v11: PASS fixed=6 retry=0 Stage18=false")
    return 0
