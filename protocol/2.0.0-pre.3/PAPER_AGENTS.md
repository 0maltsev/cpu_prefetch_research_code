...
- Before the first valid warm-up freeze, calibration and pilot runs are marked
  `PRE_FREEZE_BOOTSTRAP` and use exactly the registered five-second floor.
  Complete valid pilot evidence then freezes the later duration as the upward
  integer-tick rounding of `max(5 s, 10*h_max/rho_min)` using independent
  run-level horizons and accepted-count/measurement-horizon rates. Missing or
  invalid evidence blocks the freeze; confirmatory behavior never changes it.
