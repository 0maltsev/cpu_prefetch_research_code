#!/usr/bin/env python3
"""Production authority-verifying Stage 17 fixed-action controller v3.
...
import stage17_fixed_action_executor_v3 as executor
import stage17_output_registry_v3 as output_registry
import stage17_phase_controller_v2 as predecessor
...
def prepare_action(
    *, repository_root: pathlib.Path, journal: pathlib.Path,
    ...
    authorization, authorization_bytes = predecessor._load(authorization_path)
    predecessor._validate(root, AUTH_SCHEMA, authorization, "phase authorization")
    ...
    if authority_resolution is not None:
        ...
        if (admitted.semantic_context.get("authorization_sha256")
                != predecessor._sha(authorization_bytes)
    ...
    predecessor._verify_signature(authorization_bytes, signature_path, trust)
    issued = predecessor._parse_utc(authorization["issued_at_utc"])
    expires = predecessor._parse_utc(authorization["expires_at_utc"])
    sampled = predecessor._parse_utc(actual_now)
    ...
    request_path, request_bytes = predecessor._read_binding(
        authorization["request_binding"], "fixed request"
    )
    request = json.loads(request_bytes)
    if not isinstance(request, dict):
        raise ControllerError("fixed request root is not an object")
    predecessor._validate(root, REQUEST_SCHEMA, request, "fixed request")
...
def main() -> int:
    ...
    try:
        ...
        print(f"stage17-phase-controller-v3: FAIL: {exception}", file=sys.stderr)
        return 1
    print("stage17-phase-controller-v3: PASS action=COMPLETED authority=STAGE17_ONLY")
    return 0
