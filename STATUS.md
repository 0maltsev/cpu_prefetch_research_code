...
| Stage 2/16 implementation-decision record | `COMPLETE`; pilot-governance successor recorded | ADR-0001 through ADR-0086, ADR-0093 through ADR-0097, ADR-0099, and ADR-0100 through ADR-0109 are accepted. ADR-0105 implements append-only state persistence; ADR-0106 adds the immutable default-deny semantic predecessor; ADR-0107 adds fixed action semantics and transition/one-shot gating; ADR-0108 remains the immutable actual-clock/durable-one-shot predecessor; ADR-0109 adds the policy-v5 final/post-marker-clock and sealed OpenSSH-input-consumption successor without changing ADR-0104's graph or any v1-v4 predecessor identity. D-099..D-108 decision/evidence bytes are hash-preserved; D-105..D-108 remain proposed/unaccepted. |
...
| Stage 17 operational authorization | `PREPARED_EXTERNAL_INPUTS_REQUIRED` | ADR-0104 provides the only successor graph; ADR-0105 preserves the v1 successor/checklist and computes state from the append-only journal; ADR-0106 through ADR-0108 remain immutable predecessors. ADR-0109 policy v5/plan v3 require all long preparation before a final authority clock, a second nondecreasing live clock after durable marker and immediately before first transport, exact loaded runtime v2 closure, six precompiled programs, and sealed `memfd` snapshots for known-hosts and transport-identity bytes consumed through inherited descriptors. Local `ssh -G`, a real local child-read test, two runtime positives, seven focused positive, five focused negative, and all 79 predecessor negatives pass. `S17-EXT-006` retains exact-byte verification; the other eight inputs fail closed. The checked-in journal has 0 resolutions and 0 transitions, all ten inputs are missing, `action_ready=false`, `pilot_ready=false`, and `stand=NOT_ACCESSED`. No stand, preflight, phase, measurement, or Stage 18 authority exists. |
...
deadline. ADR-0109 preserves that complete predecessor and makes policy v5 the
current production admission. Executor v3 finishes semantic/schema/runtime
checks, six-program render/compile, and sealed `memfd` pinning before the final
UTC sample; it samples again after durable marker immediately before first
transport and rejects expiry, future authority, and rollback without opening
transport. OpenSSH consumes only inherited `/proc/self/fd/N` snapshots, never
the mutable owner path after marker. Journal runtime v2 is explicitly
hash-bound because policy v4's unversioned journal remains immutable.
