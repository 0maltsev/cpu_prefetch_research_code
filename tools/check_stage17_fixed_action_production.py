...
import stage17_exit_state_machine_v3 as exit_machine
import stage17_output_registry_v3 as output_registry
import stage17_pilot_candidate_artifact_v3 as release_artifact
import check_stage17_pilot_semantics_v3 as pilot_plan_builder
import stage17_phase_controller_v1 as rejected_controller
import stage17_phase_controller_v3 as controller
import stage17_q15_session_controller_v1 as q15_session
import stage17_semantic_verifier_v11 as semantic_registry
import stage17_state_journal_v9 as journal
...
class ExternalBuilder(legacy.Builder):
    ...
    def __init__(self, base: pathlib.Path) -> None:
        ...
        for name in ("journal", "records", "receipts", "actions", "manifests"):
            (self.operational / name).mkdir(mode=0o700)
        self.latest_path = self.operational / (
            "journal/stage17-state-journal-000000.json"
        )
        shutil.copyfile(
            self.root / "config/stage17/journal/stage17-state-journal-000000.json",
            self.latest_path,
        )
    ...
    def append(self, kind: str, reference: dict[str, Any]) -> None:
        ...
        document["previous_journal"] = {
            "path": previous_path.relative_to(self.operational).as_posix(),
            "sha256": digest(previous_path),
        }
        document[kind].append(reference)
        path = self.operational / "journal" / (
    ...
    def external_receipt(
        ...
        receipt = {
            ...
            "receipt_id": f"SYNTHETIC-{input_id}-RECEIPT-v3",
            "artifact_locator": str(artifact),
            "artifact_size_bytes": artifact.stat().st_size,
            "artifact_sha256": digest(artifact),
            "sidecars": [{"locator": str(item),
                          "size_bytes": item.stat().st_size,
                          "sha256": digest(item)} for item in (sidecars or [])],
            "custody_domain_id": "SYNTHETIC-TEST-ONLY-CUSTODY",
            "verifier_id": verifier_id, "verifier_version": verifier_version,
            "verification_result": "PASS", "verified_at_utc": legacy.utc(1),
    ...
    def add_resolution(
        ...
        document = {
            ...
            "resolution_id": f"SYNTHETIC-{input_id}-RESOLUTION-v3-TEST-ONLY",
            "sequence_number": sequence, "input_id": input_id,
            "actor": "synthetic-stage17-owner",
            "recorded_at_utc": legacy.utc(sequence + 1),
            "graph_sha256": self.graph_sha, "catalog_sha256": self.catalog_sha,
        ...
        path = self.operational / "records" / f"resolution-{sequence:03d}.json"
        write(path, document)
        value = digest(path)
        self.append("resolution_records", {
            "path": path.relative_to(self.operational).as_posix(),
            "sha256": value,
        })
    ...
    def add_transition(self) -> None:
        ...
        evidence = [{
            "input_id": item,
            "resolution_id": self.resolutions[item][1]["resolution_id"],
            "sha256": self.resolutions[item][2],
        ...
        path = self.operational / "records" / f"transition-{sequence:03d}.json"
        write(path, document)
        value = digest(path)
        self.append("transition_records", {
            "path": path.relative_to(self.operational).as_posix(),
            "sha256": value,
        })
...
class Fixture:
    def __init__(
        ...
        self.builder = ExternalBuilder(temporary)
    ...
    def typed(
        ...
        write(path, {
            "schema_version": "cpu-prefetch-stage17-operational-typed-record/3",
    ...
    def manifest(
        ...
        path = directory / f"{input_id.lower()}-manifest-v3.json"
        write(path, {
            "schema_version": "cpu-prefetch-stage17-operational-input-manifest/3",
            "manifest_id": f"SYNTHETIC-{input_id}-MANIFEST-v3-TEST-ONLY",
            "input_id": input_id,
    ...
    def ext002(self) -> None:
        ...
        provenance_document = release_artifact.build_extracted_release_receipt_v3(
            bundle_root=release_root,
            receipt_id="SYNTHETIC-EXT002-CLEAN-RELEASE-v3",
        )
        provenance = directory / "runtime-release-provenance-v3.json"
        ...
        artifacts = [
            ...
            self.artifact(
                ...
                provenance,
                schema="config/schemas/stage17-runtime-release-provenance-v3.schema.json",
            ),
        ]
    ...
    def ext003(self) -> None:
        directory = self.root / "evidence/ext003-v3"
        directory.mkdir(parents=True)
        ext2_path, ext2_doc, ext2_sha = self.builder.resolutions["S17-EXT-002"]
        ext2_manifest = self.external / "ext002/s17-ext-002-manifest-v3.json"
    ...
    def prepare_action(
        ...
        evidence_root_binding = {
            "absolute_path": str(directory.resolve()), "device": metadata.st_dev,
        ...
        request = {
            ...
            "request_id": f"SYNTHETIC-{action_id}-REQUEST-v3",
            "action_id": action_id, "stand_id": "SYNTHETIC-STAND-NOT-ACCESSED",
            "authorization_id": f"SYNTHETIC-{action_id}-AUTH-v3",
            "attempt_id": f"SYNTHETIC-{action_id}-ATTEMPT-v3",
        ...
        request_path = directory / f"{action_id.lower().replace('-', '_')}-request-v3.json"
        ...
        authorization_path = directory / (
            f"{action_id.lower().replace('-', '_')}-authorization-v3.json"
        )
    ...
    def action(
        ...
        outcome = controller.execute_once(
            repository_root=self.root, journal=self.builder.latest_path,
            journal_directory=self.builder.operational / "journal",
            operational_evidence_root=self.builder.operational,
            authorization_path=authorization_path, signature_path=signature,
            pilot_archive=self.builder.pilot_archive,
    ...
    def ext004_and_ext005(self) -> None:
        ...
        directory.mkdir(mode=0o700)
        q15_r_inputs = {
            "qualification_id": "SYNTHETIC-Q15-QUALIFICATION-v3",
            "attempt_id": "SYNTHETIC-Q15-R-ATTEMPT-v3",
            "session_id": "SYNTHETIC-Q15-SESSION-v3",
        ...
        def admit_q15_r_then_prepare_q15_w() -> controller.PreparedAction:
            q15_r_action = {
                ...
                "attempt": directory / "stage17-q15_r-attempt-v3.json",
                "result": directory / "stage17-q15-r-result-v3.json",
            }
            ...
            common = {
                ...
                "stand_id": "SYNTHETIC-STAND-NOT-ACCESSED",
                "binding_id": "SYNTHETIC-Q15-R-BINDING-v3",
            ...
            platform_path = directory / "platform-v3.json"
            write(platform_path, {
                "schema_version": "2.0.0-pre.2", "protocol_version": "2.0.0-pre.2",
                ...
                "topology": {"sockets": 1, "numa_nodes": 1,
            ...
            artifacts.append(self.artifact(
                ...
                schema="protocol/2.0.0-pre.2/handoff/schemas/platform.schema.json",
            ))
            ...
            q15_r_attempt = directory / "stage17-q15_r-attempt-v3.json"
            q15_r_result = directory / "stage17-q15-r-result-v3.json"
            q15_w_inputs = {
        ...
        q15_w_action = {
            ...
            "attempt": directory / "stage17-q15_w-attempt-v3.json",
            "result": directory / "stage17-q15-w-result-v3.json",
        ...
        mixed = self.root / "evidence/ext005-mixed-index-v3.json"
        write(mixed, {
    ...
    def ext006(self) -> None:
        contract = self.root / "evidence/synthetic-ext006-contract-v3.json"
        contract.parent.mkdir(parents=True, exist_ok=True)
        write(contract, release_artifact.build_contract_v3(
            ...
            contract_id="SYNTHETIC-EXT006-CONTRACT-v3-TEST-ONLY",
        ))
    ...
    def ext008(self) -> None:
        directory = self.root / "evidence/ext008-v3"
        directory.mkdir(parents=True)
        validation = self.validate()
        ...
        plan_path = directory / "stage17-pilot-plan-v3.json"
        write(plan_path, plan)
        manifest = self.manifest("S17-EXT-008", directory, [
            self.artifact(
                directory, "EXT008-PILOT-PLAN-v3", "PILOT_PLAN_V3",
                plan_path, schema="config/schemas/stage17-pilot-plan-v3.schema.json",
            ),
        ], self.predecessors(7))
    ...
    def ext010(self) -> None:
        directory = self.root / "evidence/ext010-v3"; directory.mkdir(parents=True)
        validation = self.validate()
        registry = output_registry.pin_registry(self.root)
        ...
        predecessors = [{"input_id": item,
                        ...
                        for item in definition["required_resolution_ids"]]
        now = dt.datetime.now(dt.timezone.utc).replace(microsecond=0)
        request_placeholder = directory / "pilot-request-v3.json"
        ...
        request = {
            "schema_version": "cpu-prefetch-stage17-fixed-action-request/3",
            "request_id": "SYNTHETIC-PILOT-REQUEST-v3",
            "action_id": "STAGE17-BLINDED-PILOT",
            "stand_id": "SYNTHETIC-STAND-NOT-ACCESSED",
            "authorization_id": "SYNTHETIC-PILOT-AUTH-v3",
            "attempt_id": "SYNTHETIC-PILOT-ATTEMPT-v3",
        ...
        auth = directory / "pilot-authorization-v3.json"; write(auth, authorization)
    ...
    def execute_pilot(self) -> dict[str, Any]:
        directory = self.root / "evidence/ext010-v3"
        outcome = controller.execute_once(
            repository_root=self.root,
            journal=self.builder.latest_path,
            journal_directory=self.builder.operational / "journal",
            operational_evidence_root=self.builder.operational,
            authorization_path=directory / "pilot-authorization-v3.json",
            signature_path=directory / "pilot-authorization-v3.json.sig",
            pilot_archive=self.builder.pilot_archive,
            pilot_sidecar=self.builder.pilot_sidecar,
            synthetic_test_only=True,
        )
        self.positive_actions += 1
        return {
            "authorization": directory / "pilot-authorization-v3.json",
            "attempt": outcome.attempt_path, "result": outcome.result_path,
            "completion": outcome.completion_path, "result_document": outcome.result,
        }
...
def exercise_exit_and_phase18(
    ...
    directory = fixture.external / "stage17-exit-v3"
    directory.mkdir(mode=0o700)
    completion = exit_machine.validate_stage17_completion(
        repository_root=fixture.root, operational_validation=validation,
        pilot_output_root=pilot["result"].parent, synthetic_test_only=True,
    )
    completion_path = directory / "stage17-completion-v3.json"
    completion_path.write_bytes(completion.payload)
    ...
    allowed.chmod(0o600)
    ext2 = validation.resolutions["S17-EXT-002"]
    ext3 = validation.resolutions["S17-EXT-003"]
    trust_context = ext3.semantic_context["trust"]
    now = dt.datetime.now(dt.timezone.utc).replace(microsecond=0)
    enrollment = {
        "schema_version": "cpu-prefetch-phase18-trust-enrollment/3",
        "enrollment_id": "SYNTHETIC-PHASE18-TRUST-ENROLLMENT-v3",
        "stage17_completion_sha256": hashlib.sha256(
            completion.payload
        ).hexdigest(),
        "stage17_trust_context": {
            "ext002_resolution_id": ext2.resolution_id,
            "ext002_resolution_sha256": ext2.sha256,
            "ext003_resolution_id": ext3.resolution_id,
            "ext003_resolution_sha256": ext3.sha256,
            "trust_record_sha256": trust_context["measurements"][
                "allowed_signers_sha256"
            ],
        },
        "independent_allowed_signers": file_binding(allowed),
        "principal": "synthetic-phase18-owner",
        "namespace": "cpu-prefetch-phase18-access-v3",
    ...
    enrollment_path = phase / "phase18-trust-enrollment-v3.json"
    write(enrollment_path, enrollment)
    enrollment_signature = fixture._sign(enrollment_path)
    trust = exit_machine.admit_phase18_trust(
        repository_root=fixture.root, operational_validation=validation,
        completion=completion, enrollment_path=enrollment_path,
        enrollment_signature_path=enrollment_signature,
    )
    readiness = exit_machine.phase18_readiness(
        repository_root=fixture.root, completion=completion, trust=trust,
    )
    readiness_path = phase / "phase18-readiness-v3.json"
    write(readiness_path, readiness)
    authorization = {
        "schema_version": "cpu-prefetch-phase18-authorization/3",
        "authorization_id": "SYNTHETIC-PHASE18-AUTHORIZATION-v3",
        "actor": enrollment["principal"],
        "reviewer": enrollment["reviewer_role"],
        "trust_enrollment_sha256": hashlib.sha256(
            trust.enrollment_payload
        ).hexdigest(),
        "stage17_completion_sha256": hashlib.sha256(
            completion.payload
        ).hexdigest(),
    ...
    authorization_path = phase / "phase18-authorization-v3.json"
    write(authorization_path, authorization)
    signature = _sign_with(
        independent_key, enrollment["namespace"], authorization_path
    )
    ...
    for sequence, (source, target) in enumerate(
        ...
        previous = hashlib.sha256(exit_machine.canonical(transition)).hexdigest()
    if exit_machine.validate_phase18_chronology(
            authorization=admitted, transitions=transitions) != "ARCHIVED":
    ...
    same_anchor = copy.deepcopy(enrollment)
    stage17_allowed = pathlib.Path(
        trust_context["measurements"]["allowed_signers_path"]
    )
    same_anchor["independent_allowed_signers"] = file_binding(stage17_allowed)
    same_path = phase / "self-rooted-enrollment-v3.json"
    write(same_path, same_anchor)
    same_signature = fixture._sign(same_path)
    expect_failure(
        "phase18_coherently_self_rooted_chain",
        lambda: exit_machine.admit_phase18_trust(
            repository_root=fixture.root, operational_validation=validation,
            completion=completion, enrollment_path=same_path,
            enrollment_signature_path=same_signature,
        ),
    ); negative += 1
    reordered = copy.deepcopy(transitions)
    reordered[0]["to_state"] = "TRAINING_OPEN"
    expect_failure(
        "phase18_chronology_skip",
        lambda: exit_machine.validate_phase18_chronology(
            authorization=admitted, transitions=reordered
        ),
    ); negative += 1
    stage17_reuse = copy.deepcopy(transitions)
    stage17_reuse[0]["stage17_authority_used"] = True
    expect_failure(
        "phase18_stage17_authority_reuse",
        lambda: exit_machine.validate_phase18_chronology(
            authorization=admitted, transitions=stage17_reuse
        ),
    ); negative += 1
    return 4, negative
...
def _manifest_path(fixture: Fixture, input_id: str) -> pathlib.Path:
    if input_id in {"S17-EXT-003", "S17-EXT-008", "S17-EXT-010"}:
        return fixture.root / "evidence" / {
            "S17-EXT-003": "ext003-v3/s17-ext-003-manifest-v3.json",
            "S17-EXT-008": "ext008-v3/s17-ext-008-manifest-v3.json",
            "S17-EXT-010": "ext010-v3/s17-ext-010-manifest-v3.json",
        }[input_id]
    return fixture.external / {
        "S17-EXT-002": "ext002/s17-ext-002-manifest-v3.json",
        "S17-EXT-004": "q15-session/s17-ext-004-manifest-v3.json",
        "S17-EXT-005": "q15-session/s17-ext-005-manifest-v3.json",
        "S17-EXT-007": "ext007/s17-ext-007-manifest-v3.json",
        "S17-EXT-009": "ext009/s17-ext-009-manifest-v3.json",
    }[input_id]
...
def semantic_regressions(
    ...
    def verify_manifest(input_id: str) -> dict[str, Any]:
        ...
        return semantics.verify_manifest_v3(
    ...
    malicious = pathlib.Path("/bin/true").read_bytes() + (
        b"\0--execute-fixed-stage17-action-v3\0"
        b"STAGE17-FIXED-ACTION-WORKER-v3\0"
    )
    ...
    output_reference["schema_identity"] = "cpu-prefetch-stage17-run-output/3"
...
def _copy_projection(destination: pathlib.Path) -> pathlib.Path:
    projection = destination / "repository"
    ignored = {".git", "build", "evidence", "__pycache__", ".cache"}

    def ignore(_: str, names: list[str]) -> set[str]:
        return {name for name in names if name in ignored or name.endswith(".pyc")}

    shutil.copytree(ROOT, projection, symlinks=False, ignore=ignore)
    return projection


def _outer(args: argparse.Namespace) -> int:
    with tempfile.TemporaryDirectory(prefix="stage17-dry-run-projection-") as temporary:
        base = pathlib.Path(temporary)
        projection = _copy_projection(base)
        inputs = base / "inputs"
        inputs.mkdir(mode=0o700)
        copied: dict[str, pathlib.Path] = {}
        for name, source in {
            "worker": args.worker, "no_result_worker": args.no_result_worker,
            "bundle_archive": args.bundle_archive,
            "bundle_sidecar": args.bundle_sidecar,
        }.items():
            ...
            if os.access(source, os.X_OK):
                target.chmod(0o700)
            copied[name] = target
        extracted_parent = base / "bundle"
        extracted_parent.mkdir()
        bundle_root = release_artifact._safe_extract(
            copied["bundle_archive"], extracted_parent
        )
        command = [
            sys.executable, "-B",
            str(projection / "tools/check_stage17_fixed_action_production.py"),
            "--self-test", "--internal",
            "--worker", str(copied["worker"]),
            "--no-result-worker", str(copied["no_result_worker"]),
            "--bundle-root", str(bundle_root),
            "--bundle-archive", str(copied["bundle_archive"]),
            "--bundle-sidecar", str(copied["bundle_sidecar"]),
        ]
        completed = subprocess.run(
            command, cwd=projection, stdin=subprocess.DEVNULL,
...
def main() -> int:
    ...
    if args.bundle_root is None:
        parser.error("--internal requires --bundle-root")
    try:
