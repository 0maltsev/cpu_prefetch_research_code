...
| Stage 17 operational authorization | `PREPARED_EXTERNAL_INPUTS_REQUIRED` | ADR-0104 provides the only successor graph; ADR-0105 preserves the v1 successor/checklist and computes state from the append-only journal; ADR-0106 remains the immutable v2 evidence predecessor. ADR-0107 policy v3 removes owner-controlled command bytes, hash-binds the fixed plan and production verifier/executor/collector, validates the Ed25519 wire blob and exact known-hosts bytes, requires byte-identical executable files and a safe evidence root, and permits `action_ready=true` only at exact transition 1 with live authority and no prior attempt. `S17-EXT-006` retains exact-byte verification; the other eight inputs fail closed. The checked-in journal has 0 resolutions and 0 transitions, all ten inputs are missing, `action_ready=false`, `pilot_ready=false`, and `stand=NOT_ACCESSED`. No stand, preflight, phase, measurement, or Stage 18 authority exists. |
...
blocked. ADR-0107 policy v3 fixes every preflight command/argv/stdin/output,
hash-binds its plan and implementations, validates Ed25519/known-hosts/
executable/root semantics, and requires transition 1 plus a live unconsumed
one-shot authorization for action readiness. Canonical replay of the checked-in genesis is
