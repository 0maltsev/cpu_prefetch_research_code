#!/usr/bin/env python3
"""Stage 17 read-only preflight runtime-cardinality semantic successor."""
...
POLICY_PATH = "config/stage17/stage17-read-only-preflight-evidence-admission-policy-v12.json"
POLICY_SCHEMA_PATH = "config/schemas/stage17-read-only-preflight-evidence-admission-policy-v12.schema.json"
POLICY_V11_PATH = (
    "config/stage17/stage17-read-only-preflight-"
    "evidence-admission-policy-v11.json"
)
ADR_0121_PATH = "docs/decisions/0121-stage17-preflight-marker-runtime-cardinality.md"
...
ENVELOPE_SCHEMA_PATH = "config/schemas/stage17-operational-evidence-envelope-v12.schema.json"
AUTHORIZATION_SCHEMA_PATH = "config/schemas/stage17-read-only-preflight-authorization-v10.schema.json"
CONTRACT_SCHEMA_PATH = "config/schemas/stage17-read-only-preflight-supporting-contract-v10.schema.json"
ATTEMPT_SCHEMA_PATH = "config/schemas/stage17-read-only-preflight-attempt-v8.schema.json"
...
IMPLEMENTATION_PATHS = {
    "semantic_verifier": "tools/stage17_read_only_preflight_semantic_verifier_v12.py",
    "executor": "tools/stage17_read_only_preflight_executor_v10.py",
    "collector": predecessor.COLLECTOR_PATH,
    "state_journal": "tools/stage17_state_journal_v14.py",
...
def verify_policy_v12(
    *, root: pathlib.Path, policy: dict[str, Any], graph_sha256: str,
    ...
    _validate_schema(root, policy, POLICY_SCHEMA_PATH, "semantic policy v12")
    expected_predecessor = {
        "policy_v11": _binding_for(root, POLICY_V11_PATH),
        "adr_0121": _binding_for(root, ADR_0121_PATH),
    ...
    if policy.get("predecessor") != expected_predecessor:
        raise SemanticAdmissionError("semantic policy v12 predecessor binding drifted")
    if policy.get("schema_bindings") != [_binding_for(root, item) for item in SCHEMA_PATHS]:
        raise SemanticAdmissionError("semantic policy v12 schema binding drifted")
    if policy.get("fixed_action_plan") != _binding_for(root, EFFECTIVE_ACTION_PLAN_PATH):
        raise SemanticAdmissionError("semantic policy v12 effective action plan drifted")
    if policy.get("successor_action_plan") != _binding_for(root, SUCCESSOR_ACTION_PLAN_PATH):
        raise SemanticAdmissionError("semantic policy v12 successor action plan drifted")
    if policy.get("operational_contract") != _binding_for(root, "config/stage17/stage17-operational-input-external-contract-v1.json"):
        raise SemanticAdmissionError("semantic policy v12 operational contract drifted")
    if policy.get("implementations") != {
        name: _binding_for(root, path) for name, path in IMPLEMENTATION_PATHS.items()
    }:
        raise SemanticAdmissionError("semantic policy v12 runtime closure drifted")
    ...
    if policy.get("operational_implementations") != expected_operational_implementations:
        raise SemanticAdmissionError("semantic policy v12 operational runtime drifted")
    ...
    if policy.get("operational_definitions") != expected_operational_definitions:
        raise SemanticAdmissionError("semantic policy v12 operational definitions drifted")
    ...
    if entries != expected:
        raise SemanticAdmissionError("semantic policy v12 complete registry drifted")
    if policy.get("synthetic_bypass_available") is not False or policy.get("stage18_authority") is not False:
        raise SemanticAdmissionError("semantic policy v12 authority boundary drifted")
...
def verify_s17_ext_001_semantics_v12(
    *, root: pathlib.Path, resolution: dict[str, Any],
    ...
    envelopes = [(path, document) for path, document in repository_documents if document.get("schema_version") == "cpu-prefetch-stage17-operational-evidence-envelope/12"]
    if len(envelopes) != 1 or len(repository_documents) != 1:
        raise SemanticAdmissionError("S17-EXT-001 requires exactly one v12 envelope")
    envelope_path, envelope = envelopes[0]
    _validate_schema(root, envelope, ENVELOPE_SCHEMA_PATH, "S17-EXT-001 v12 envelope")
    ...
    expected_predecessor = {
        ...
        "resolution_schema_sha256": resolution_schema_sha256,
        "semantic_policy_v11_sha256": policy["predecessor"]["policy_v11"]["sha256"],
        "adr_0121_sha256": policy["predecessor"]["adr_0121"]["sha256"],
    }
    if envelope.get("predecessor") != expected_predecessor:
        raise SemanticAdmissionError("S17-EXT-001 v12 predecessor binding drifted")
    ...
    if envelope["authorization"].get("schema_identity") != "cpu-prefetch-stage17-read-only-preflight-authorization/10" or envelope["supporting_contract"].get("schema_identity") != "cpu-prefetch-stage17-read-only-preflight-supporting-contract/10":
    ...
    _validate_schema(root, authorization, AUTHORIZATION_SCHEMA_PATH, "authorization v10")
    _validate_schema(root, contract, CONTRACT_SCHEMA_PATH, "supporting contract v10")
    ...
    verified["attempt_marker_name"] = "stage17-read-only-preflight-attempt-v8.json"
...
def evaluate_s17_ext_001_action_readiness_v12(
