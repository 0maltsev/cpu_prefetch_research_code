#!/usr/bin/env python3
"""Stage 17 registry with the D-121 pre-marker contract successor."""
...
import stage17_read_only_preflight_semantic_verifier_v12 as preflight
import stage17_semantic_verifier_v15 as predecessor
...
def verify_policy_v16(*, root: pathlib.Path, policy: dict[str, Any],
    ...
    if errors:
        raise SemanticAdmissionError(f"policy v16 schema rejection: {errors[0].message}")
    ...
    expected = [{"input_id": item, "status": "IMPLEMENTED",
                 "verifier_id": VERIFIER_ID,
                 "verifier_version": VERIFIER_VERSION} for item in INPUT_IDS]
    ...
    discovered = discover_python_closure(root, tuple(policy["runtime_import_roots"]))
    ...
    required = {"tools/stage17_state_journal_v14.py",
    ...
    if not required.issubset(discovered):
        raise SemanticAdmissionError("policy v16 runtime closure is incomplete")


def verify_policy_v14(**arguments: Any) -> None:
    verify_policy_v16(**arguments)
...
def _verify_ext001(**arguments: Any) -> dict[str, Any]:
    ...
    try:
        ...
        return preflight.verify_s17_ext_001_semantics_v12(
