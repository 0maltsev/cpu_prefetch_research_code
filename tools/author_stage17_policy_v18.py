#!/usr/bin/env python3
"""Reproducibly render or verify Stage 17 production policy v18."""
...
import author_stage17_policy_v17 as predecessor
import stage17_semantic_verifier_v18 as verifier


ROOTS = (
    "stage17_state_journal_v16",
    "stage17_read_only_preflight_executor_v12",
    "stage17_phase_controller_v8",
    "stage17_q15_session_controller_v6",
    "stage17_operational_cli_v10",
...
SUCCESSOR_BINDINGS = (
    ...
    "config/stage17/stage17-operational-evidence-admission-policy-v17.json",
    "config/stage17/stage17-read-only-preflight-evidence-admission-policy-v14.json",
    ...
    "tools/stage17_semantic_verifier_v18.py",
    "tools/stage17_state_journal_v16.py",
    "tools/stage17_phase_controller_v8.py",
    "tools/stage17_q15_session_controller_v6.py",
    "tools/stage17_operational_cli_v10.py",
    "tools/author_stage17_action_revalidation_blocker_v1.py",
)
...
def render(root: pathlib.Path) -> dict[str, object]:
    ...
    policy.update({
        "schema_version":
            "cpu-prefetch-stage17-operational-evidence-admission-policy/18",
        "policy_id": "STAGE17-OPERATIONAL-EVIDENCE-ADMISSION-POLICY-v18",
        "policy_version": "18",
        "predecessor": predecessor.predecessor.predecessor.predecessor.binding(
            root,
            "config/stage17/stage17-operational-evidence-admission-policy-v17.json",
        ),
        "predecessor_status":
            "SUPERSEDED_PRE_MARKER_ACTION_REVALIDATION_SCHEMA_KEY_INCOMPATIBLE",
        "runtime_import_roots": list(ROOTS),
        "runtime_closure": {
            pathlib.PurePosixPath(path).stem:
                predecessor.predecessor.predecessor.predecessor.binding(root, path)
...
def main() -> int:
    ...
    canonical = predecessor.predecessor.predecessor.predecessor.canonical
    payload = canonical(render(root))
    destination = root / verifier.POLICY_PATH
    if arguments.check:
        if not destination.is_file() or destination.read_bytes() != payload:
            print("stage17-policy-v18: FAIL: generated policy bytes drifted",
                  file=sys.stderr)
            return 1
        print("stage17-policy-v18: PASS")
        return 0
    if arguments.write:
        ...
        try:
            ...
            os.close(directory)
        print(f"stage17-policy-v18: PASS written={destination}")
        return 0
