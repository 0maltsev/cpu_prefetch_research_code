#!/usr/bin/env python3
"""Stage 17 registry binding executor v13's action-readiness fix (ADR-0126).

This is a new, additive file.  It does not modify
`stage17_semantic_verifier_v19.py`'s bytes, so the already-accepted-shaped
`stage17-operational-evidence-admission-policy-v19.json` runtime-closure
hash for that file remains valid and unchanged.  `S17-EXT-001` resolution-
time semantics are delegated unchanged to
`stage17_read_only_preflight_semantic_verifier_v15.verify_s17_ext_001_semantics_v15`
-- that logic did not change.  The only real change here is which executor
the policy's `runtime_closure` requires: `stage17_read_only_preflight_
executor_v13.py`, whose action-time call site actually reaches
`evaluate_s17_ext_001_action_readiness_v15` instead of a nonexistent
`_v14` attribute on the new preflight verifier module.
"""
...
import stage17_semantic_verifier_v19 as predecessor
...
def verify_policy_v20(*, root: pathlib.Path, policy: dict[str, Any],
    ...
    if errors:
        raise SemanticAdmissionError(
            f"policy v20 schema rejection: {errors[0].message}"
        )
    ...
    if not required.issubset(discovered):
        raise SemanticAdmissionError("policy v20 runtime closure is incomplete")
    if "tools/stage17_read_only_preflight_executor_v12.py" in discovered:
        raise SemanticAdmissionError(
            "policy v20 must bind executor v13, not the superseded v12 entry point"
        )


def verify_policy_v14(**arguments: Any) -> None:
    verify_policy_v20(**arguments)
