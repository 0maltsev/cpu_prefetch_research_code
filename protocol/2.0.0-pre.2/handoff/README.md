...
Protocol version: **`2.0.0-pre.2`**. Primary data and lifecycle instances are incompatible with `1.x`; the closed `2.0.0-pre.2` run-manifest representation is also incompatible with `2.0.0-pre.1` final dispositions. Prior snapshots remain readable and immutable; see `AMENDMENTS.md`.
