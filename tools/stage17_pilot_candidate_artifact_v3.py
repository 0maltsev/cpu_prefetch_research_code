#!/usr/bin/env python3
"""Full-bundle EXT006 v3 verifier and contract authoring support."""
...
VERIFIER_VERSION = "3"
SCHEMA = "config/schemas/stage17-pilot-candidate-external-contract-v3.schema.json"
SUPPORTED_ACTIONS = (
...
def verify_extracted_bundle_v3(bundle_root: pathlib.Path) -> ExtractedReleaseContext:
    """Verify a clean extracted v3 bundle and derive its immutable runtime class.
    ...
    if profile not in {
        ...
        raise ArtifactError("extracted bundle profile is not a v3 Stage 17 profile")
    synthetic = profile == "STAGE17-HERMETIC-DRY-RUN-BUNDLE-v1"
    expected_runtime = {
    ...
    if (payload[:4] != b"\x7fELF"
            or b"--execute-fixed-stage17-action-v3" not in payload
            or b"STAGE17-FIXED-ACTION-WORKER-v3" not in payload):
...
def build_extracted_release_receipt_v3(
    *, bundle_root: pathlib.Path, receipt_id: str,
) -> dict[str, Any]:
    context = verify_extracted_bundle_v3(bundle_root)
    relative_worker = context.worker_path.relative_to(context.bundle_root).as_posix()
    result = {
        "schema_version": "cpu-prefetch-stage17-runtime-release-provenance/3",
...
def verify_extracted_release_receipt_v3(
    *, repository_root: pathlib.Path, receipt: dict[str, Any],
) -> ExtractedReleaseContext:
    schema = _load_json(
        repository_root / "config/schemas/stage17-runtime-release-provenance-v3.schema.json"
    )
    ...
    context = verify_extracted_bundle_v3(pathlib.Path(receipt["bundle_root"]))
    expected = build_extracted_release_receipt_v3(
...
def build_contract_v3(
    *, repository_root: pathlib.Path, archive: pathlib.Path, sidecar: pathlib.Path,
    ...
    with tempfile.TemporaryDirectory(prefix="cpu-prefetch-ext006-author-") as temporary:
        ...
        extracted_context = verify_extracted_bundle_v3(extracted)
        ...
        result = {
            "schema_version": "cpu-prefetch-stage17-pilot-candidate-external-contract/3",
...
def verify_pilot_candidate_artifact_v3(
    *, repository_root: pathlib.Path, contract_path: pathlib.Path,
    ...
    if errors:
        raise ArtifactError(f"EXT006 v3 contract schema rejection: {errors[0].message}")
    ...
    with tempfile.TemporaryDirectory(prefix="cpu-prefetch-ext006-verify-") as temporary:
        ...
        if (payload[:4] != b"\x7fELF" or
                b"--execute-fixed-stage17-action-v3" not in payload or
                b"STAGE17-FIXED-ACTION-WORKER-v3" not in payload):
            raise ArtifactError("EXT006 worker is not the fixed v3 dispatcher")
