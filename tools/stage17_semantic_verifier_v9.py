#!/usr/bin/env python3
"""Current Stage 17 semantic policy verifier (all ten inputs)."""
...
POLICY_PATH = "config/stage17/stage17-operational-evidence-admission-policy-v9.json"
POLICY_SCHEMA_PATH = "config/schemas/stage17-operational-evidence-admission-policy-v9.schema.json"
POLICY_V8_PATH = "config/stage17/stage17-operational-evidence-admission-policy-v8.json"
ADR_0113_PATH = "docs/decisions/0113-stage17-pid-namespace-and-complete-operational-admission.md"
...
ENVELOPE_SCHEMA_PATH = "config/schemas/stage17-operational-evidence-envelope-v9.schema.json"
...
SCHEMA_PATHS = (
    ...
    predecessor.AUTHORIZATION_SCHEMA_PATH, predecessor.CONTRACT_SCHEMA_PATH,
    predecessor.PLAN_SCHEMA_PATH, SUCCESSOR_ACTION_PLAN_SCHEMA_PATH,
    ATTEMPT_SCHEMA_PATH, predecessor.RECEIPT_SCHEMA_PATH,
    ...
    ACTION_AUTHORIZATION_SCHEMA_PATH, ACTION_RESULT_SCHEMA_PATH,
...
IMPLEMENTATION_PATHS = {
    "semantic_verifier": "tools/stage17_semantic_verifier_v9.py",
    "executor": "tools/stage17_read_only_preflight_executor_v7.py",
    "collector": predecessor.COLLECTOR_PATH,
    "state_journal": "tools/stage17_state_journal_v6.py",
    ...
    "state_journal_v5_helper": "tools/stage17_state_journal_v5.py",
...
def verify_policy_v9(
    *, root: pathlib.Path, policy: dict[str, Any], graph_sha256: str,
    ...
    _validate_schema(root, policy, POLICY_SCHEMA_PATH, "semantic policy v9")
    expected_predecessor = {
        "policy_v8": _binding_for(root, POLICY_V8_PATH),
        "adr_0113": _binding_for(root, ADR_0113_PATH),
    ...
    if policy.get("operational_implementations") != expected_operational_implementations:
        raise SemanticAdmissionError("semantic policy v9 operational runtime drifted")
    ...
    if policy.get("operational_definitions") != expected_operational_definitions:
        raise SemanticAdmissionError("semantic policy v9 operational definitions drifted")
    ...
    if entries != expected:
        raise SemanticAdmissionError("semantic policy v9 complete registry drifted")
    if policy.get("synthetic_bypass_available") is not False or policy.get("stage18_authority") is not False:
        raise SemanticAdmissionError("semantic policy v9 authority boundary drifted")
...
def verify_s17_ext_001_semantics_v9(
    *, root: pathlib.Path, resolution: dict[str, Any],
    ...
    envelopes = [(path, document) for path, document in repository_documents if document.get("schema_version") == "cpu-prefetch-stage17-operational-evidence-envelope/9"]
    if len(envelopes) != 1 or len(repository_documents) != 1:
        raise SemanticAdmissionError("S17-EXT-001 requires exactly one v9 envelope")
    envelope_path, envelope = envelopes[0]
    _validate_schema(root, envelope, ENVELOPE_SCHEMA_PATH, "S17-EXT-001 v9 envelope")
    ...
    expected_predecessor = {
        ...
        "resolution_schema_sha256": resolution_schema_sha256,
        "semantic_policy_v8_sha256": policy["predecessor"]["policy_v8"]["sha256"],
        "adr_0113_sha256": policy["predecessor"]["adr_0113"]["sha256"],
    }
    if envelope.get("predecessor") != expected_predecessor:
        raise SemanticAdmissionError("S17-EXT-001 v9 predecessor binding drifted")
    ...
    if envelope["authorization"].get("schema_identity") != "cpu-prefetch-stage17-read-only-preflight-authorization/8" or envelope["supporting_contract"].get("schema_identity") != "cpu-prefetch-stage17-read-only-preflight-supporting-contract/8":
    ...
    _validate_schema(root, authorization, predecessor.AUTHORIZATION_SCHEMA_PATH, "authorization v8")
    _validate_schema(root, contract, predecessor.CONTRACT_SCHEMA_PATH, "supporting contract v8")
    ...
    # replacing only the attempt schema/name changed by v9.
...
def evaluate_s17_ext_001_action_readiness_v9(
