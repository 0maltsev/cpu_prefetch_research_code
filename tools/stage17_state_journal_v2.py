#!/usr/bin/env python3
"""Versioned Stage 17 journal runtime for semantic-admission policy v5.

The immutable v1 graph, record schemas, journal snapshots, and mechanics stay
in :mod:`stage17_state_journal`. This successor changes only the active
semantic-policy/verifier seam and action-readiness projection.
"""
...
from stage17_semantic_verifier_v5 import (
    ...
    verify_policy_v5,
    verify_s17_ext_001_semantics_v5,
)
...
SEMANTIC_POLICY_PATH = pathlib.PurePosixPath(
    "config/stage17/stage17-operational-evidence-admission-policy-v5.json"
)
SEMANTIC_POLICY_SCHEMA_PATH = pathlib.PurePosixPath(
    "config/schemas/stage17-operational-evidence-admission-policy-v5.schema.json"
)
SEMANTIC_ENVELOPE_SCHEMA_PATH = pathlib.PurePosixPath(
    "config/schemas/stage17-operational-evidence-envelope-v5.schema.json"
)
S17_EXT_001_AUTHORIZATION_SCHEMA_PATH = pathlib.PurePosixPath(
    "config/schemas/stage17-read-only-preflight-authorization-v5.schema.json"
)
S17_EXT_001_CONTRACT_SCHEMA_PATH = pathlib.PurePosixPath(
    "config/schemas/stage17-read-only-preflight-supporting-contract-v5.schema.json"
)
