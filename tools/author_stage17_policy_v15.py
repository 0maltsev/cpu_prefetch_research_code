#!/usr/bin/env python3
"""Reproducibly render or verify Stage 17 production policy v15."""
...
import author_stage17_policy_v14 as predecessor
import stage17_semantic_verifier_v15 as verifier


ROOTS = (
    "stage17_state_journal_v13",
    "stage17_read_only_preflight_executor_v9",
    "stage17_phase_controller_v5",
    "stage17_q15_session_controller_v3",
    "stage17_operational_cli_v7",
...
SUCCESSOR_BINDINGS = (
    "docs/decisions/0120-stage17-post-preflight-controller-compatibility.md",
    ...
    "tools/stage17_semantic_verifier_v15.py",
...
def render(root: pathlib.Path) -> dict[str, object]:
    ...
    policy.update({
        "schema_version":
            "cpu-prefetch-stage17-operational-evidence-admission-policy/15",
        "policy_id": "STAGE17-OPERATIONAL-EVIDENCE-ADMISSION-POLICY-v15",
        "policy_version": "15",
        ...
        "runtime_closure": {
            pathlib.PurePosixPath(path).stem: predecessor.binding(root, path)
...
def main() -> int:
    ...
    payload = predecessor.canonical(render(root))
    destination = root / verifier.POLICY_PATH
    if arguments.check:
        if not destination.is_file() or destination.read_bytes() != payload:
            print("stage17-policy-v15: FAIL: generated policy bytes drifted",
                  file=sys.stderr)
            return 1
        print("stage17-policy-v15: PASS")
        return 0
    if arguments.write:
        ...
        try:
            ...
            os.close(directory)
        print(f"stage17-policy-v15: PASS written={destination}")
        return 0
