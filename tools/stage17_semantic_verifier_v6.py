#!/usr/bin/env python3
"""Fail-closed Stage 17 S17-EXT-001 semantic verifier, policy version 6."""
...
VERIFIER_VERSION = "6"
POLICY_V5_PATH = "config/stage17/stage17-operational-evidence-admission-policy-v5.json"
...
ACTION_PLAN_PATH = "config/stage17/stage17-read-only-preflight-fixed-action-plan-v4.json"
EXECUTOR_PATH = "tools/stage17_read_only_preflight_executor_v4.py"
COLLECTOR_PATH = "tools/stage17_read_only_preflight_collector_v2.py"
STATE_JOURNAL_PATH = "tools/stage17_state_journal_v3.py"
SNAPSHOT_BROKER_PATH = "tools/stage17_openssh_parent_snapshot_v1.py"
AUTHORIZATION_SCHEMA_PATH = "config/schemas/stage17-read-only-preflight-authorization-v6.schema.json"
CONTRACT_SCHEMA_PATH = "config/schemas/stage17-read-only-preflight-supporting-contract-v6.schema.json"
ENVELOPE_SCHEMA_PATH = "config/schemas/stage17-operational-evidence-envelope-v6.schema.json"
PLAN_SCHEMA_PATH = "config/schemas/stage17-read-only-preflight-fixed-action-plan-v4.schema.json"
ATTEMPT_SCHEMA_PATH = "config/schemas/stage17-read-only-preflight-attempt-v4.schema.json"
RECEIPT_SCHEMA_PATH = "config/schemas/stage17-read-only-preflight-observation-receipt-v3.schema.json"
FAILURE_SCHEMA_PATH = "config/schemas/stage17-read-only-preflight-failure-v4.schema.json"
COMPLETION_SCHEMA_PATH = "config/schemas/stage17-read-only-preflight-completion-v3.schema.json"
POLICY_SCHEMA_PATH = "config/schemas/stage17-operational-evidence-admission-policy-v6.schema.json"
V6_SCHEMA_PATHS = (
    POLICY_SCHEMA_PATH,
...
IMPLEMENTATION_PATHS = {
    "semantic_verifier": "tools/stage17_semantic_verifier_v6.py",
...
ATTEMPT_NAME = "stage17-read-only-preflight-attempt-v4.json"
...
def verify_policy_v6(
    *,
    ...
    _validate_schema(root, policy, POLICY_SCHEMA_PATH, "semantic policy v6")
    expected_predecessor = {
        "policy_v5": _binding_for(root, POLICY_V5_PATH),
    ...
    if policy.get("predecessor") != expected_predecessor:
        raise SemanticAdmissionError("semantic policy v6 predecessor binding drifted")
    if policy.get("schema_bindings") != [
        _binding_for(root, item) for item in V6_SCHEMA_PATHS
    ]:
        raise SemanticAdmissionError("semantic policy v6 schema binding drifted")
    if policy.get("fixed_action_plan") != _binding_for(root, ACTION_PLAN_PATH):
        raise SemanticAdmissionError("semantic policy v6 action-plan binding drifted")
    ...
    if policy.get("implementations") != expected_implementations:
        raise SemanticAdmissionError("semantic policy v6 runtime binding drifted")
    entries = policy.get("entries", [])
    expected_ids = [f"S17-EXT-{index:03d}" for index in range(1, 11)]
    if [entry.get("input_id") for entry in entries] != expected_ids:
        raise SemanticAdmissionError("semantic policy v6 registry drifted")
    if entries[0] != {
        ...
        raise SemanticAdmissionError("S17-EXT-001 v6 registration drifted")
...
def _verify_action_plan(
    ...
    path = _binding(root, policy["fixed_action_plan"], "fixed action plan v4")
    plan = _load_json(path)
    _validate_schema(root, plan, PLAN_SCHEMA_PATH, "fixed action plan v4")
    if plan.get("predecessor") != _binding_for(
        root, "config/stage17/stage17-read-only-preflight-fixed-action-plan-v3.json"
    ):
        raise SemanticAdmissionError("fixed action plan v4 predecessor drifted")
    ...
    if (
        ...
        raise SemanticAdmissionError("fixed action plan v4 transport drifted")
    ...
    if set(plan.get("forbidden_semantics", {}).values()) != {True}:
        raise SemanticAdmissionError("fixed action plan v4 lost a prohibition")
    if plan.get("authority_boundary") != PERMISSIONS:
        raise SemanticAdmissionError("fixed action plan v4 widened authority")
...
def _verify_contract_action_inputs(
    ...
    expected_plan = {
        **policy["fixed_action_plan"],
        "schema_identity": "cpu-prefetch-stage17-read-only-preflight-fixed-action-plan/4",
    }
...
def verify_s17_ext_001_semantics_v6(
    *, root: pathlib.Path, resolution: dict[str, Any],
    ...
    envelopes = [(path, document) for path, document in repository_documents if
        document.get("schema_version") == "cpu-prefetch-stage17-operational-evidence-envelope/6"]
    if len(envelopes) != 1 or len(repository_documents) != 1:
        raise SemanticAdmissionError("S17-EXT-001 requires exactly one v6 envelope")
    _, envelope = envelopes[0]
    _validate_schema(root, envelope, ENVELOPE_SCHEMA_PATH, "S17-EXT-001 v6 envelope")
    ...
    if envelope["predecessor"] != {
        ...
        "resolution_schema_sha256": resolution_schema_sha256,
        "semantic_policy_v5_sha256": policy["predecessor"]["policy_v5"]["sha256"],
    ...
    if envelope["authorization"].get("schema_identity") != "cpu-prefetch-stage17-read-only-preflight-authorization/6":
        raise SemanticAdmissionError("authorization schema identity mismatch")
    authorization = _load_json(authorization_path)
    _validate_schema(root, authorization, AUTHORIZATION_SCHEMA_PATH, "authorization v6")
    contract_path = _binding(root, envelope["supporting_contract"], "supporting contract")
    if envelope["supporting_contract"].get("schema_identity") != "cpu-prefetch-stage17-read-only-preflight-supporting-contract/6":
        raise SemanticAdmissionError("supporting-contract schema identity mismatch")
    contract = _load_json(contract_path)
    _validate_schema(root, contract, CONTRACT_SCHEMA_PATH, "supporting contract v6")
    ...
    if authorization["fixed_action_plan"] != expected_plan:
        raise SemanticAdmissionError("authorization does not bind action plan v4")
    ...
    expected_scope = (f"STAND_ID={target['stand_id']};SSH_TARGET={target['ssh_target']};"
        "SCOPE=READ_ONLY_PREFLIGHT;PLAN=STAGE17-READ-ONLY-PREFLIGHT-FIXED-ACTION-PLAN-v4")
...
def reverify_action_inputs(
    ...
    fixed_names = [
        *(f"stage17-read-only-preflight-attempt-v{version}.json" for version in range(1, 5)),
        "stage17-read-only-preflight-failure-v2.json",
        "stage17-read-only-preflight-failure-v3.json", FAILURE_NAME,
        "stage17-read-only-preflight-completion-v1.json",
        "stage17-read-only-preflight-completion-v2.json", COMPLETION_NAME,
    ]
    fixed_names.extend(f"s17-ro-{ordinal:03d}{suffix}" for ordinal in range(1, 7)
        for suffix in (".stdout.bin", ".stderr.bin", ".receipt-v1.json", ".receipt-v2.json", ".receipt-v3.json"))
