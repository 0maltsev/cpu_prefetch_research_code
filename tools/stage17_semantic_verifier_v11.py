#!/usr/bin/env python3
"""Current Stage 17 closed semantic registry (policy v11)."""
...
import stage17_operational_semantics_v3 as operational
import stage17_pilot_candidate_artifact_v3 as release_verifier
import stage17_semantic_verifier_v10 as predecessor
    ...
    *, root: pathlib.Path, policy: dict[str, Any], graph_sha256: str,
    ...
    if errors:
        raise SemanticAdmissionError(
            f"policy v11 schema rejection: {errors[0].message}"
        )
    ...
    if policy["predecessor_status"] != "REJECTED_INCOMPLETE_PRODUCTION_BOUNDARY":
        raise SemanticAdmissionError("policy v10 predecessor rejection is absent")
...
def verify_s17_ext_006(
    *, root: pathlib.Path, receipt_documents: list[dict[str, Any]],
    ...
    try:
        context = release_verifier.verify_pilot_candidate_artifact_v3(
...
def verify_operational_manifest(
    *, input_id: str, root: pathlib.Path,
    ...
    candidates = [path for path, document in repository_documents
                  ...
                  "cpu-prefetch-stage17-operational-input-manifest/3"]
    ...
    if len(candidates) != 1:
        raise SemanticAdmissionError(f"{input_id} requires exactly one v3 manifest")
    path = candidates[0]
    ...
    try:
        context = operational.verify_manifest_v3(
