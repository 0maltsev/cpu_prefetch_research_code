#!/usr/bin/env python3
"""Stage 17 journal runtime bound to semantic policy v23.

Additive successor to `stage17_state_journal_v18.py`: the only change is
which semantic-verifier module the reassignment loop propagates to the
predecessor journal chain. Policy v22 (ADR-0128) overrides `S17-EXT-006`
dispatch; policy v23 (ADR-0129) additionally overrides `S17-EXT-002`
dispatch so real evidence captured from a bundle newer than `v4` admits
correctly. Without this file, v23's `S17-EXT-002` fix is unreachable from
real `admit-resolution` calls, since v18 (and everything importing it)
stays hardcoded to v22 -- the same reachability gap class already found
and fixed once for v17/v19 during the v21 promotion.
"""
...
import stage17_semantic_verifier_v23 as semantic
