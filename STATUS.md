...
| Stage 2/16 implementation-decision record | `COMPLETE`; D-116 accepted | ADR-0001 through ADR-0122 are recorded. ADR-0113/policy v9 is `REJECTED_FAIL_OPEN_PREDECESSOR`; ADR-0114/policy v10 and ADR-0115/policy v11 are immutable incomplete predecessors. ADR-0116 accepts the pre-freeze warm-up bootstrap in protocol pre.3; ADR-0117/policy v12 remains the durable-pilot predecessor. ADR-0118 through ADR-0121 preserve earlier runtime/transaction boundaries. ADR-0122 prospectively fixes only the D-121 OpenSSH-fixture/terminal-record compatibility defect and issues no authority. D-099..D-108 decision/evidence bytes remain hash-preserved. |
...
| Stage 17 operational authorization | `D120_PRE_MARKER_AND_D121_POST_MARKER_STOPS_PRESERVED; D122_SOFTWARE_REQUIRES_FRESH_RELEASE_AND_AUTHORITY; EXTERNAL_INPUTS_REQUIRED` | D-120 remains the typed pre-marker cardinality stop. The authorized D-121 successor created attempt v8, then stopped before `Popen`: the local OpenSSH capability fixture left two adopted descendants under PID 1, and the 17-key terminal schemas could not retain the 20-key runtime failure. ADR-0122 preserves that marker-only transaction, records a typed no-retry post-marker blocker, and binds preflight policy v13/attempt v9/executor v11 plus operational policy v17/journal v15/controller v7/CLI v9. Snapshot broker v2 establishes a real OpenSSH/sshd adopted-child reap barrier before marker; all terminal records require the same exact 22 named runtime identities. A fresh clean release and separately issued finite S17-EXT-001 are required; no D-121 retry is allowed. Q15/Q16/pilot still require ordered real evidence and separate authority. Checked-in state remains 0 resolutions, 0 transitions, ten missing, `action_ready=false`, `pilot_ready=false`, `stand=NOT_ACCESSED`. |
