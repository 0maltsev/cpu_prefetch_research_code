#!/usr/bin/env python3
"""Fail-closed Stage 17 S17-EXT-001 semantic verifier, policy version 7."""
...
VERIFIER_VERSION = "7"
POLICY_V6_PATH = "config/stage17/stage17-operational-evidence-admission-policy-v6.json"
ADR_0110_PATH = "docs/decisions/0110-stage17-openssh-consumed-snapshot-locator.md"
ADR_0111_PATH = "docs/decisions/0111-stage17-authority-and-ssh-lifecycle-boundary.md"
ACTION_PLAN_PATH = "config/stage17/stage17-read-only-preflight-fixed-action-plan-v5.json"
EXECUTOR_PATH = "tools/stage17_read_only_preflight_executor_v5.py"
COLLECTOR_PATH = "tools/stage17_read_only_preflight_collector_v2.py"
STATE_JOURNAL_PATH = "tools/stage17_state_journal_v4.py"
SNAPSHOT_BROKER_PATH = "tools/stage17_openssh_parent_snapshot_v1.py"
AUTHORIZATION_SCHEMA_PATH = "config/schemas/stage17-read-only-preflight-authorization-v7.schema.json"
CONTRACT_SCHEMA_PATH = "config/schemas/stage17-read-only-preflight-supporting-contract-v7.schema.json"
ENVELOPE_SCHEMA_PATH = "config/schemas/stage17-operational-evidence-envelope-v7.schema.json"
PLAN_SCHEMA_PATH = "config/schemas/stage17-read-only-preflight-fixed-action-plan-v5.schema.json"
ATTEMPT_SCHEMA_PATH = "config/schemas/stage17-read-only-preflight-attempt-v5.schema.json"
RECEIPT_SCHEMA_PATH = "config/schemas/stage17-read-only-preflight-observation-receipt-v4.schema.json"
FAILURE_SCHEMA_PATH = "config/schemas/stage17-read-only-preflight-failure-v5.schema.json"
COMPLETION_SCHEMA_PATH = "config/schemas/stage17-read-only-preflight-completion-v4.schema.json"
POLICY_SCHEMA_PATH = "config/schemas/stage17-operational-evidence-admission-policy-v7.schema.json"
V7_SCHEMA_PATHS = (
    POLICY_SCHEMA_PATH,
...
IMPLEMENTATION_PATHS = {
    "semantic_verifier": "tools/stage17_semantic_verifier_v7.py",
...
ATTEMPT_NAME = "stage17-read-only-preflight-attempt-v5.json"
...
def verify_policy_v7(
    *,
    ...
    _validate_schema(root, policy, POLICY_SCHEMA_PATH, "semantic policy v7")
    expected_predecessor = {
        "policy_v6": _binding_for(root, POLICY_V6_PATH),
        "adr_0110": _binding_for(root, ADR_0110_PATH),
    ...
    if policy.get("predecessor") != expected_predecessor:
        raise SemanticAdmissionError("semantic policy v7 predecessor binding drifted")
    if policy.get("schema_bindings") != [
        _binding_for(root, item) for item in V7_SCHEMA_PATHS
    ]:
        raise SemanticAdmissionError("semantic policy v7 schema binding drifted")
    if policy.get("fixed_action_plan") != _binding_for(root, ACTION_PLAN_PATH):
        raise SemanticAdmissionError("semantic policy v7 action-plan binding drifted")
    ...
    if policy.get("implementations") != expected_implementations:
        raise SemanticAdmissionError("semantic policy v7 runtime binding drifted")
    entries = policy.get("entries", [])
    expected_ids = [f"S17-EXT-{index:03d}" for index in range(1, 11)]
    if [entry.get("input_id") for entry in entries] != expected_ids:
        raise SemanticAdmissionError("semantic policy v7 registry drifted")
    if entries[0] != {
        ...
        raise SemanticAdmissionError("S17-EXT-001 v7 registration drifted")
...
def _verify_action_plan(
    ...
    path = _binding(root, policy["fixed_action_plan"], "fixed action plan v5")
    plan = _load_json(path)
    _validate_schema(root, plan, PLAN_SCHEMA_PATH, "fixed action plan v5")
    if plan.get("predecessor") != _binding_for(
        root, "config/stage17/stage17-read-only-preflight-fixed-action-plan-v4.json"
    ):
        raise SemanticAdmissionError("fixed action plan v5 predecessor drifted")
    ...
    if (
        ...
        raise SemanticAdmissionError("fixed action plan v5 transport drifted")
    ...
    if set(plan.get("forbidden_semantics", {}).values()) != {True}:
        raise SemanticAdmissionError("fixed action plan v5 lost a prohibition")
    if plan.get("authority_boundary") != PERMISSIONS:
        raise SemanticAdmissionError("fixed action plan v5 widened authority")
...
def _verify_contract_action_inputs(
    ...
    expected_plan = {
        **policy["fixed_action_plan"],
        "schema_identity": "cpu-prefetch-stage17-read-only-preflight-fixed-action-plan/5",
    }
...
def verify_s17_ext_001_semantics_v7(
    *, root: pathlib.Path, resolution: dict[str, Any],
    ...
    envelopes = [(path, document) for path, document in repository_documents if
        document.get("schema_version") == "cpu-prefetch-stage17-operational-evidence-envelope/7"]
    if len(envelopes) != 1 or len(repository_documents) != 1:
        raise SemanticAdmissionError("S17-EXT-001 requires exactly one v7 envelope")
    _, envelope = envelopes[0]
    _validate_schema(root, envelope, ENVELOPE_SCHEMA_PATH, "S17-EXT-001 v7 envelope")
    ...
    if envelope["predecessor"] != {
        ...
        "resolution_schema_sha256": resolution_schema_sha256,
        "semantic_policy_v6_sha256": policy["predecessor"]["policy_v6"]["sha256"],
        "adr_0110_sha256": policy["predecessor"]["adr_0110"]["sha256"],
    ...
    if envelope["authorization"].get("schema_identity") != "cpu-prefetch-stage17-read-only-preflight-authorization/7":
        raise SemanticAdmissionError("authorization schema identity mismatch")
    authorization = _load_json(authorization_path)
    _validate_schema(root, authorization, AUTHORIZATION_SCHEMA_PATH, "authorization v7")
    contract_path = _binding(root, envelope["supporting_contract"], "supporting contract")
    if envelope["supporting_contract"].get("schema_identity") != "cpu-prefetch-stage17-read-only-preflight-supporting-contract/7":
        raise SemanticAdmissionError("supporting-contract schema identity mismatch")
    contract = _load_json(contract_path)
    _validate_schema(root, contract, CONTRACT_SCHEMA_PATH, "supporting contract v7")
    ...
    if authorization["fixed_action_plan"] != expected_plan:
        raise SemanticAdmissionError("authorization does not bind action plan v5")
    ...
    expected_scope = (f"STAND_ID={target['stand_id']};SSH_TARGET={target['ssh_target']};"
        "SCOPE=READ_ONLY_PREFLIGHT;PLAN=STAGE17-READ-ONLY-PREFLIGHT-FIXED-ACTION-PLAN-v5")
...
def reverify_action_inputs(
    ...
    fixed_names = [
        *(f"stage17-read-only-preflight-attempt-v{version}.json" for version in range(1, 6)),
        ...
        "stage17-read-only-preflight-failure-v4.json", FAILURE_NAME,
        "stage17-read-only-preflight-completion-v1.json",
        "stage17-read-only-preflight-completion-v2.json",
        "stage17-read-only-preflight-completion-v3.json", COMPLETION_NAME,
    ]
    fixed_names.extend(f"s17-ro-{ordinal:03d}{suffix}" for ordinal in range(1, 7)
        for suffix in (".stdout.bin", ".stderr.bin", ".receipt-v1.json", ".receipt-v2.json", ".receipt-v3.json", ".receipt-v4.json"))
