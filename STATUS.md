...
| Stage 2/16 implementation-decision record | `COMPLETE`; pilot-governance successor recorded | ADR-0001 through ADR-0086, ADR-0093 through ADR-0097, ADR-0099, and ADR-0100 through ADR-0111 are accepted. ADR-0105 implements append-only state persistence; ADR-0106 adds the immutable default-deny semantic predecessor; ADR-0107 adds fixed action semantics and transition/one-shot gating; ADR-0108 through ADR-0110 remain immutable runtime/clock/snapshot predecessors. ADR-0111 adds the policy-v7 final authority guard and process-group ownership/reap successor without changing ADR-0104's graph or any predecessor identity. D-099..D-108 decision/evidence bytes are hash-preserved; D-105..D-108 remain proposed/unaccepted. |
...
| Stage 17 operational authorization | `PREPARED_EXTERNAL_INPUTS_REQUIRED` | ADR-0104 provides the only successor graph; ADR-0105 preserves the v1 successor/checklist and computes state from the append-only journal; ADR-0106 through ADR-0110 remain immutable predecessors. ADR-0111 policy v7/plan v5 bind verifier v7, executor v5, journal runtime v4, unchanged collector v2 and snapshot broker v1, plus every record/schema/helper byte. The final live system/monotonic authority guard is inside the transport boundary after all snapshot verification and immediately before `Popen`; every started child owns a new process group and is terminated/reaped before snapshots close or failure evidence is written. Six focused positives and twenty-five focused negatives pass, including the characterized v4 expiry/child-leak defects, all requested post-`Popen` faults, process-group cleanup, real local OpenSSH in-place and atomic replacement of both credential sources, concurrency, and marker versions v1-v5. The preserved 17A.5 8/20, 17A.4 7/5, two runtime positives, and 79 predecessor negatives remain green. `S17-EXT-006` retains exact-byte verification; the other eight inputs fail closed. The checked-in journal has 0 resolutions and 0 transitions, all ten inputs are missing, `action_ready=false`, `pilot_ready=false`, and `stand=NOT_ACCESSED`. No stand, preflight, phase, measurement, or Stage 18 authority exists. |
...
current without changing any predecessor. Executor v5 moves both snapshot
