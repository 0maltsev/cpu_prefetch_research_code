#!/usr/bin/env python3
"""Stage 17 external-journal read-only preflight executor successor.
...
import stage17_read_only_preflight_semantic_verifier_v10 as current_semantic
import stage17_state_journal
import stage17_state_journal_v10
import stage17_state_journal_v11 as current_journal


EXECUTOR_ID = "STAGE17-READ-ONLY-PREFLIGHT-EXECUTOR-v8"
...
def runtime_identity_paths() -> dict[str, str]:
    """Return the exact loaded preflight-policy-v10 runtime closure."""
...
def _prospective_validation(
    *, repository_root: pathlib.Path, latest_journal: pathlib.Path,
    ...
    if (
        ...
        action_context = current_semantic.evaluate_s17_ext_001_action_readiness_v10(
...
def main() -> int:
    ...
    try:
        ...
        print(f"stage17-read-only-preflight-v8: FAIL: {exception}", file=sys.stderr)
        return 1
    print("stage17-read-only-preflight-v8: PASS fixed=6 retry=0 Stage18=false")
    return 0
