#!/usr/bin/env python3
"""Reproducibly render or verify the Stage 17 production policy v14."""
...
import stage17_semantic_verifier_v14 as verifier
import stage17_state_journal as journal


ROOTS = (
    "stage17_state_journal_v12",
    "stage17_read_only_preflight_executor_v9",
...
BINDING_PATHS = (
    ...
    "docs/decisions/0118-stage17-external-journal-preflight-runtime.md",
    "docs/decisions/0119-stage17-preflight-canonical-serializer-runtime.md",
    "config/stage17/stage17-fixed-phase-actions-v4.json",
    "config/stage17/stage17-operational-evidence-admission-policy-v13.json",
    "config/stage17/stage17-read-only-preflight-evidence-admission-policy-v11.json",
    "config/schemas/stage17-operational-evidence-admission-policy-v14.schema.json",
    "config/schemas/stage17-read-only-preflight-evidence-admission-policy-v11.schema.json",
    "config/schemas/stage17-operational-evidence-envelope-v11.schema.json",
    "config/schemas/stage17-read-only-preflight-authorization-v9.schema.json",
    "config/schemas/stage17-read-only-preflight-supporting-contract-v9.schema.json",
    ...
    "tools/stage17_operational_cli_v6.py",
...
def render(root: pathlib.Path) -> dict[str, Any]:
    ...
    predecessor = binding(
        root, "config/stage17/stage17-operational-evidence-admission-policy-v13.json"
    )
    return {
        "schema_version": "cpu-prefetch-stage17-operational-evidence-admission-policy/14",
        "policy_id": "STAGE17-OPERATIONAL-EVIDENCE-ADMISSION-POLICY-v14",
        "policy_version": "14",
        "protocol_version": "2.0.0-pre.3",
        "predecessor": predecessor,
        "predecessor_status": "SUPERSEDED_MISSING_CANONICAL_SERIALIZER_IMPORT",
...
def main() -> int:
    ...
    if arguments.check:
        ...
        if not current.is_file() or current.read_bytes() != payload:
            print("stage17-policy-v14: FAIL: generated policy bytes drifted", file=sys.stderr)
            return 1
        print("stage17-policy-v14: PASS")
        return 0
    if arguments.write:
        ...
        try:
            ...
            os.close(directory)
        print(f"stage17-policy-v14: PASS written={destination}")
        return 0
