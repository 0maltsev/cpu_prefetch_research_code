#!/usr/bin/env python3
"""Fail-closed Stage 17 operational CLI with terminal-compatibility preflight successor.
...
import stage17_phase_controller_v7 as controller
...
import stage17_state_journal_v15 as journal_runtime
...
def author_ext001(
    *, repository_root: pathlib.Path, evidence_root: pathlib.Path,
    ...
    pre_marker_blocker: pathlib.Path, post_marker_blocker: pathlib.Path,
    ...
    successor = {
        **policy["successor_action_plan"],
        "schema_identity":
            "cpu-prefetch-stage17-read-only-preflight-fixed-action-plan/8",
    }
    limits = {
    ...
    contract = {
        "schema_version":
            "cpu-prefetch-stage17-read-only-preflight-supporting-contract/11",
    ...
    contract_path = destination / "supporting-contract-v11.json"
    ...
    envelope = {
        "schema_version": "cpu-prefetch-stage17-operational-evidence-envelope/13",
        ...
        "predecessor": {
            ...
            "semantic_policy_v12_sha256":
                policy["predecessor"]["policy_v12"]["sha256"],
    ...
    envelope_path = destination / "envelope-v13.json"
    validate_schema(
        repository_root,
        "config/schemas/stage17-read-only-preflight-supporting-contract-v11.schema.json",
        contract,
    )
    validate_schema(
        repository_root,
    ...
    validate_schema(
        repository_root,
        "config/schemas/stage17-operational-evidence-envelope-v13.schema.json",
        envelope,
    )
    ...
    try:
        ...
        if arguments.command == "init":
            ...
            result = author_ext001(
                ...
                post_marker_blocker=arguments.post_marker_blocker.resolve(),
