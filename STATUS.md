...
| Stage 17 operational authorization | `LOCAL_ENGINEERING_CLOSED; PREPARED_EXTERNAL_INPUTS_REQUIRED` | ADR-0117/policy v12 supplies the durable pilot, Q15/Q16, exit, and Phase 18 boundary. ADR-0118/policy v13 additionally binds CLI v5, executor v8, and the external-root journal runtime after the first real T1 handoff stopped before marker/transport because executor v7 required a repository-contained journal. The successor preserves exact repository definitions while validating append-only journal/resolution/transition bytes below the distinct owner-mode evidence root. Focused regression: 2 positive/3 negative, including predecessor characterization and cross-root rejection, with zero marker/transport. All ten checked-in inputs remain absent: 0 resolutions, 0 transitions, `action_ready=false`, `pilot_ready=false`, `stand=NOT_ACCESSED`. |
