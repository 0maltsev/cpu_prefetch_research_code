#!/usr/bin/env python3
"""Stage 17 closed semantic registry with external-journal preflight runtime (policy v14)."""
...
import stage17_semantic_verifier_v13 as predecessor
import stage17_read_only_preflight_semantic_verifier_v11 as preflight
    ...
    *, root: pathlib.Path, policy: dict[str, Any], graph_sha256: str,
    ...
    if errors:
        raise SemanticAdmissionError(
            f"policy v14 schema rejection: {errors[0].message}"
        )
    ...
    if policy["predecessor_status"] != \
            "SUPERSEDED_MISSING_CANONICAL_SERIALIZER_IMPORT":
        raise SemanticAdmissionError("policy v13 predecessor status is absent")
...
def verify_s17_ext_001(**arguments: Any) -> dict[str, Any]:
    ...
    try:
        ...
        return preflight.verify_s17_ext_001_semantics_v11(
            **{
                key: value
