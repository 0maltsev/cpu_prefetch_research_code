...
| Stage 17 operational authorization | `D120_PRE_MARKER_STOP_PRESERVED; D121_SOFTWARE_READY_FOR_FRESH_RELEASE_AND_READ_ONLY_PREFLIGHT; EXTERNAL_INPUTS_REQUIRED` | The real D-120 transaction reached T1 but stopped before marker/transport because attempt schema v7 admitted 17 runtime hashes while policy v11 supplied 19. ADR-0121 preserves that journal, authorization, and empty output root; a typed blocker receipt proves no attempt/transport/retry and cannot grant authority. Preflight policy v12, attempt v8, executor v10, operational policy v16, journal v14, controller v6 and CLI v8 bind an exact closed 20-key runtime set. Focused 4-positive/5-negative and disk-backed public-CLI replay reach `AUTHORIZED_FOR_READ_ONLY_PREFLIGHT` with 20 identities and zero marker/transport. A fresh clean release and fresh finite S17-EXT-001 are required; Q15/Q16/pilot still require ordered real evidence and separate authority. Checked-in state remains 0 resolutions, 0 transitions, ten missing, `action_ready=false`, `pilot_ready=false`, `stand=NOT_ACCESSED`. |
