#!/usr/bin/env python3
"""Versioned Stage 17 journal runtime for semantic-admission policy v6."""
...
from stage17_semantic_verifier_v6 import (
    evaluate_s17_ext_001_action_readiness,
    verify_policy_v6,
    verify_s17_ext_001_semantics_v6,
)
...
SEMANTIC_POLICY_PATH = pathlib.PurePosixPath(
    "config/stage17/stage17-operational-evidence-admission-policy-v6.json"
)
SEMANTIC_POLICY_SCHEMA_PATH = pathlib.PurePosixPath(
    "config/schemas/stage17-operational-evidence-admission-policy-v6.schema.json"
)
SEMANTIC_ENVELOPE_SCHEMA_PATH = pathlib.PurePosixPath(
    "config/schemas/stage17-operational-evidence-envelope-v6.schema.json"
)
S17_EXT_001_AUTHORIZATION_SCHEMA_PATH = pathlib.PurePosixPath(
    "config/schemas/stage17-read-only-preflight-authorization-v6.schema.json"
)
S17_EXT_001_CONTRACT_SCHEMA_PATH = pathlib.PurePosixPath(
    "config/schemas/stage17-read-only-preflight-supporting-contract-v6.schema.json"
)
...
def load_semantic_policy(
    ...
    *,
    ...
    validate_schema(policy, root / SEMANTIC_POLICY_SCHEMA_PATH, "semantic-admission policy v6")
    try:
        verify_policy_v6(
            root=root,
            policy=policy,
...
SEMANTIC_VERIFIERS: dict[tuple[str, str, str], SemanticVerifier] = {
    ("S17-EXT-001", "STAGE17-S17-EXT-001-SEMANTIC-VERIFIER", "6"):
        verify_s17_ext_001_semantics_v6,
