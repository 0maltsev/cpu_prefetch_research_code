...
| Stage 2/16 implementation-decision record | `COMPLETE`; D-116 accepted | ADR-0001 through ADR-0112 remain immutable decisions. ADR-0113/policy v9 is `REJECTED_FAIL_OPEN_PREDECESSOR`; ADR-0114/policy v10 and ADR-0115/policy v11 are immutable incomplete predecessors. ADR-0116 accepts the pre-freeze warm-up bootstrap in protocol pre.3; ADR-0117/policy v12 remains the durable-pilot predecessor. ADR-0118/policy v13 corrects only the external-journal preflight runtime and issues no authority. D-099..D-108 decision/evidence bytes remain hash-preserved. |
...
| Stage 17 operational authorization | `READ_ONLY_PREFLIGHT_READY; POST_PREFLIGHT_CONTROLLER_BLOCKED` | ADR-0117/policy v12 supplies the durable pilot, Q15/Q16, exit, and Phase 18 design. ADR-0118/policy v13 binds CLI v5, executor v8, and the external-root journal runtime after executor v7 rejected the first real T1 handoff before marker/transport. Focused regression: 2 positive/3 negative with zero marker/transport. A first successor release then exposed omitted, policy-v10-bound compatibility modules during admission; packaging now materializes and independently verifies the entire nested preflight closure, and that partial transaction remains retained without retry. The clean full-workflow rehearsal also proved that controller v4 still loads policy v12/envelope v9 after a v10 EXT001 journal; it fails closed before Q15 and remains a later implementation blocker. Exactly one fresh read-only preflight is eligible; no Q15/Q16/pilot action is. Checked-in state remains 0 resolutions, 0 transitions, ten missing, `action_ready=false`, `pilot_ready=false`, `stand=NOT_ACCESSED`. |
...
Phase 18 access locally. ADR-0118/policy v13 supersedes only its unusable
repository-contained preflight-journal path with the accepted external-root
runtime; no action authority is embedded. The bundle-root handoff
