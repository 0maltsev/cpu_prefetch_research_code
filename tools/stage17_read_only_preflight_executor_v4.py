#!/usr/bin/env python3
"""Parent-procfd-pinned Stage 17 read-only preflight executor.
...
EXECUTOR_ID = "STAGE17-READ-ONLY-PREFLIGHT-EXECUTOR-v4"
...
def runtime_identity_paths() -> dict[str, str]:
    """Return the exact loaded policy-v6 runtime closure."""

    return {
        "semantic_verifier": str(pathlib.Path(stage17_semantic_verifier_v6.__file__).resolve()),
        ...
        "state_journal": str(pathlib.Path(stage17_state_journal_v3.__file__).resolve()),
...
def _all_one_shot_names(context: dict[str, Any]) -> list[str]:
    ...
    names.extend(
        ...
        for suffix in (
            ...
            ".receipt-v2.json", ".receipt-v3.json",
        )
    )
    return names
...
def _transport_once(
    ...
    """Open one bounded transport; no credential descriptor is inherited."""

    process = subprocess.Popen(
        list(argv), stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE,
        shell=False, env=FIXED_LOCAL_ENVIRONMENT, close_fds=True,
    )
    ...
    return TransportResult(
        returncode=returncode, stdout=bytes(output["stdout"]),
...
def _write_failure(
    *, prepared: PreparedAction, failure: OperationalFailure, completed: list[str],
    ...
    document = {
        "schema_version": "cpu-prefetch-stage17-read-only-preflight-failure/4",
...
) -> stage17_state_journal_v3.JournalValidation:
    return stage17_state_journal_v3.validate_journal(
...
def execute_once(
    *, repository_root: pathlib.Path, latest_journal: pathlib.Path,
    ...
    """Execute one admitted action using current clocks and parent snapshots."""
    ...
    try:
        ...
        context = prepared.context
        issued = stage17_state_journal_v3.parse_utc(
            context["authorization"]["issued_at_utc"], "actual authority issue"
        )
        expires = stage17_state_journal_v3.parse_utc(
            context["authorization"]["expires_at_utc"], "actual authority expiry"
        )
        prospective = stage17_state_journal_v3.parse_utc(
            prospective_utc, "prospective authority UTC"
        )
        before_marker = stage17_state_journal_v3.parse_utc(
        ...
        marker = {
            "schema_version": "cpu-prefetch-stage17-read-only-preflight-attempt/4",
            ...
            "actual_authority_sample_before_marker_utc": before_marker_utc,
            "monotonic_deadline_ns": deadline_ns, "attempt_number": 1,
        ...
        try:
            prepared.attempt_marker_sha256 = _write_exclusive_at(
                ...
                stage17_state_journal_v3.canonical_json_bytes(marker) + b"\n",
            )
            marker_created = True
        ...
        try:
            for descriptor, program in zip(prepared.program_descriptors, prepared.programs, strict=True):
                ...
                if descriptor["ordinal"] == 1:
                    try:
                        ...
                        before_transport = stage17_state_journal_v3.parse_utc(
                    ...
                    relation = _authority_relation(
                        sample=before_transport, previous=before_marker,
                        issued=issued, expires=expires,
                    )
                    if relation != "LIVE_NONDECREASING":
                        raise OperationalFailure(
                            "AUTHORITY_TIME", f"AUTHORITY_{relation}_BEFORE_FIRST_TRANSPORT",
                ...
                receipt = {
                    "schema_version": "cpu-prefetch-stage17-read-only-preflight-observation-receipt/3",
                ...
                try:
                    ...
                    receipt_hashes.append(_write_exclusive_at(
                        prepared.directory_fd, f"s17-ro-{ordinal:03d}.receipt-v3.json",
                        stage17_state_journal_v3.canonical_json_bytes(receipt) + b"\n",
                    ))
                except BaseException as exception:
                    raise OperationalFailure(
                ...
                _remaining_seconds(deadline_ns)
            completion = {
                "schema_version": "cpu-prefetch-stage17-read-only-preflight-completion/3",
            ...
            try:
                _write_exclusive_at(
                    ...
                    stage17_state_journal_v3.canonical_json_bytes(completion) + b"\n",
                )
            except BaseException as exception:
                raise OperationalFailure(
...
def main() -> int:
    ...
    except (ActionExecutionError, stage17_state_journal_v3.JournalError, OSError, ValueError) as exception:
        print(f"stage17-read-only-preflight-v4: FAIL: {exception}", file=sys.stderr)
        return 1
    print("stage17-read-only-preflight-v4: PASS fixed=6 retry=0 Stage18=false")
    return 0
