...
| Stage 2/16 implementation-decision record | `COMPLETE`; pilot-governance successor recorded | ADR-0001 through ADR-0086, ADR-0093 through ADR-0097, ADR-0099, and ADR-0100 through ADR-0108 are accepted. ADR-0105 implements append-only state persistence; ADR-0106 adds the immutable default-deny semantic predecessor; ADR-0107 adds fixed action semantics and transition/one-shot gating; ADR-0108 adds an actual-clock, literal-OpenSSH-path, loaded-runtime, durable-marker, typed-record, global-deadline v4 successor without changing ADR-0104's graph or any v1/v2/v3 predecessor identity. D-099..D-108 decision/evidence bytes are hash-preserved; D-105..D-108 remain proposed/unaccepted. |
...
| Stage 17 operational authorization | `PREPARED_EXTERNAL_INPUTS_REQUIRED` | ADR-0104 provides the only successor graph; ADR-0105 preserves the v1 successor/checklist and computes state from the append-only journal; ADR-0106 and ADR-0107 remain immutable predecessors. ADR-0108 policy v4/plan v2 require actual system UTC, literal OpenSSH paths verified with local `ssh -G`, the exact loaded runtime closure, pre-marker six-program validation, an owner-safe directory-FD evidence root, marker-file and parent-directory fsync before transport, typed attempt/receipt/failure/completion records, and one 180-second monotonic deadline. Two runtime positives and 79 negatives pass. `S17-EXT-006` retains exact-byte verification; the other eight inputs fail closed. The checked-in journal has 0 resolutions and 0 transitions, all ten inputs are missing, `action_ready=false`, `pilot_ready=false`, and `stand=NOT_ACCESSED`. No stand, preflight, phase, measurement, or Stage 18 authority exists. |
...
deadline. Canonical replay of the checked-in genesis is
