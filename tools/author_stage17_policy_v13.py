#!/usr/bin/env python3
"""Reproducibly render or verify the Stage 17 production policy v13."""
...
import stage17_semantic_verifier_v13 as verifier
import stage17_state_journal as journal


ROOTS = (
    "stage17_state_journal_v11",
    "stage17_read_only_preflight_executor_v8",
...
BINDING_PATHS = (
    ...
    "config/stage17/stage17-read-only-preflight-evidence-admission-policy-v10.json",
    "config/schemas/stage17-operational-evidence-admission-policy-v13.schema.json",
    "config/schemas/stage17-read-only-preflight-evidence-admission-policy-v10.schema.json",
    "config/schemas/stage17-operational-evidence-envelope-v10.schema.json",
    ...
    "tools/stage17_operational_cli_v5.py",
...
def render(root: pathlib.Path) -> dict[str, Any]:
    ...
    predecessor = binding(
        root, "config/stage17/stage17-operational-evidence-admission-policy-v12.json"
    )
    return {
        "schema_version": "cpu-prefetch-stage17-operational-evidence-admission-policy/13",
        "policy_id": "STAGE17-OPERATIONAL-EVIDENCE-ADMISSION-POLICY-v13",
        "policy_version": "13",
        "protocol_version": "2.0.0-pre.3",
        "predecessor": predecessor,
        "predecessor_status": "SUPERSEDED_EXTERNAL_JOURNAL_RUNTIME_DEFECT",
...
def main() -> int:
    ...
    if arguments.check:
        ...
        if not current.is_file() or current.read_bytes() != payload:
            print("stage17-policy-v13: FAIL: generated policy bytes drifted", file=sys.stderr)
            return 1
        print("stage17-policy-v13: PASS")
        return 0
    if arguments.write:
        ...
        try:
            ...
            os.close(directory)
        print(f"stage17-policy-v13: PASS written={destination}")
        return 0
