#!/usr/bin/env python3
"""Fail-closed Stage 17 S17-EXT-001 semantic verifier, policy version 8."""
...
from stage17_semantic_verifier_v5 import _exact_second, _external_private_binding


VERIFIER_ID = "STAGE17-S17-EXT-001-SEMANTIC-VERIFIER"
VERIFIER_VERSION = "8"
POLICY_V7_PATH = "config/stage17/stage17-operational-evidence-admission-policy-v7.json"
ADR_0111_PATH = "docs/decisions/0111-stage17-authority-and-ssh-lifecycle-boundary.md"
ADR_0112_PATH = "docs/decisions/0112-stage17-process-group-quiescence-and-cleanup-evidence.md"
ACTION_PLAN_PATH = "config/stage17/stage17-read-only-preflight-fixed-action-plan-v6.json"
EXECUTOR_PATH = "tools/stage17_read_only_preflight_executor_v6.py"
COLLECTOR_PATH = "tools/stage17_read_only_preflight_collector_v2.py"
STATE_JOURNAL_PATH = "tools/stage17_state_journal_v5.py"
SNAPSHOT_BROKER_PATH = "tools/stage17_openssh_parent_snapshot_v1.py"
SUPERVISOR_PATH = "tools/stage17_process_group_supervisor_v1.py"
AUTHORIZATION_SCHEMA_PATH = "config/schemas/stage17-read-only-preflight-authorization-v8.schema.json"
CONTRACT_SCHEMA_PATH = "config/schemas/stage17-read-only-preflight-supporting-contract-v8.schema.json"
ENVELOPE_SCHEMA_PATH = "config/schemas/stage17-operational-evidence-envelope-v8.schema.json"
PLAN_SCHEMA_PATH = "config/schemas/stage17-read-only-preflight-fixed-action-plan-v6.schema.json"
ATTEMPT_SCHEMA_PATH = "config/schemas/stage17-read-only-preflight-attempt-v6.schema.json"
RECEIPT_SCHEMA_PATH = "config/schemas/stage17-read-only-preflight-observation-receipt-v5.schema.json"
FAILURE_SCHEMA_PATH = "config/schemas/stage17-read-only-preflight-failure-v6.schema.json"
FAILURE_RETENTION_SCHEMA_PATH = "config/schemas/stage17-read-only-preflight-failure-retention-v1.schema.json"
COMPLETION_SCHEMA_PATH = "config/schemas/stage17-read-only-preflight-completion-v5.schema.json"
POLICY_SCHEMA_PATH = "config/schemas/stage17-operational-evidence-admission-policy-v8.schema.json"
V8_SCHEMA_PATHS = (
    POLICY_SCHEMA_PATH,
...
IMPLEMENTATION_PATHS = {
    "semantic_verifier": "tools/stage17_semantic_verifier_v8.py",
    "executor": EXECUTOR_PATH,
    "collector": COLLECTOR_PATH,
    "state_journal": STATE_JOURNAL_PATH,
    "openssh_snapshot_broker": SNAPSHOT_BROKER_PATH,
    "process_group_supervisor": SUPERVISOR_PATH,
    ...
    "state_journal_v4_helper": "tools/stage17_state_journal_v4.py",
    "state_journal_v3_helper": "tools/stage17_state_journal_v3.py",
    "state_journal_v2_helper": "tools/stage17_state_journal_v2.py",
...
ATTEMPT_NAME = "stage17-read-only-preflight-attempt-v6.json"
...
PINNING_MECHANISM = "LINUX_SEALED_MEMFD_PARENT_PROCFS-v1"
REQUIRED_SEALS = ["F_SEAL_WRITE", "F_SEAL_GROW", "F_SEAL_SHRINK", "F_SEAL_SEAL"]


def verify_policy_v8(
    *,
    root: pathlib.Path,
    policy: dict[str, Any],
    graph_sha256: str,
    ...
    _validate_schema(root, policy, POLICY_SCHEMA_PATH, "semantic policy v8")
    expected_predecessor = {
        "policy_v7": _binding_for(root, POLICY_V7_PATH),
        "adr_0111": _binding_for(root, ADR_0111_PATH),
        "adr_0112": _binding_for(root, ADR_0112_PATH),
    ...
    if policy.get("predecessor") != expected_predecessor:
        raise SemanticAdmissionError("semantic policy v8 predecessor binding drifted")
    if policy.get("schema_bindings") != [
        _binding_for(root, item) for item in V8_SCHEMA_PATHS
    ]:
        raise SemanticAdmissionError("semantic policy v8 schema binding drifted")
    if policy.get("fixed_action_plan") != _binding_for(root, ACTION_PLAN_PATH):
        raise SemanticAdmissionError("semantic policy v8 action-plan binding drifted")
    ...
    if policy.get("implementations") != expected_implementations:
        raise SemanticAdmissionError("semantic policy v8 runtime binding drifted")
    entries = policy.get("entries", [])
    expected_ids = [f"S17-EXT-{index:03d}" for index in range(1, 11)]
    if [entry.get("input_id") for entry in entries] != expected_ids:
        raise SemanticAdmissionError("semantic policy v8 registry drifted")
    if entries[0] != {
        ...
        raise SemanticAdmissionError("S17-EXT-001 v8 registration drifted")
...
def _verify_action_plan(
    ...
    path = _binding(root, policy["fixed_action_plan"], "fixed action plan v6")
    plan = _load_json(path)
    _validate_schema(root, plan, PLAN_SCHEMA_PATH, "fixed action plan v6")
    if plan.get("predecessor") != _binding_for(
        root, "config/stage17/stage17-read-only-preflight-fixed-action-plan-v5.json"
    ):
        raise SemanticAdmissionError("fixed action plan v6 predecessor drifted")
    ...
    if (
        ...
        or transport.get("pass_fds_required") is not False
        or transport.get("credential_descriptor_inheritance_forbidden") is not True
        ...
        raise SemanticAdmissionError("fixed action plan v6 transport drifted")
    if plan.get("authority_transport_boundary") != {
        "snapshot_reverification_before_final_guard": True,
        "final_system_utc_inside_transport_boundary": True,
        "popen_is_next_operation_after_guard": True,
        "system_utc_live_and_nondecreasing_required": True,
        "monotonic_authority_deadline_required": True,
        "global_deadline_and_cleanup_reserve_required": True,
        "guard_required_before_every_transport": True,
    }:
        raise SemanticAdmissionError("fixed final authority boundary drifted")
    if plan.get("ssh_child_lifecycle") != {
        "start_new_session": True,
        "single_transport_owner": True,
        "terminate_process_group_on_every_exception_timeout_or_cancellation": True,
        "bounded_sigterm_then_sigkill": True,
        "reap_before_snapshot_close": True,
        "reap_before_failure_or_receipt": True,
        "cleanup_charged_to_global_monotonic_deadline": True,
        "final_reap_barrier_overrides_return_on_budget_exhaustion": True,
        "cleanup_exception_cannot_hide_primary_failure": True,
        "linux_child_subreaper_required": True,
        "leader_exit_observed_with_waitid_wnowait": True,
        "leader_pid_and_pgid_held_until_group_quiescence": True,
        "leader_reaped_and_process_group_gone_proved_independently": True,
        "same_group_descendant_after_leader_exit_is_operational_failure": True,
        "no_signal_after_leader_reap": True,
        "failure_retention_fallback_required": True,
        "pid_or_private_bytes_in_evidence": False,
    }:
        raise SemanticAdmissionError("fixed SSH child lifecycle drifted")
    pinning = plan.get("openssh_input_pinning", {})
    expected_pinning = {
        "mechanism": PINNING_MECHANISM,
        "procfs_visible_pid_source": "NUMERIC_READLINK_OF_PROC_SELF",
        "os_getpid_as_procfs_pid_allowed": False,
        ...
        "parent_holds_descriptors_until_ssh_exit": True,
        "child_inherits_credential_descriptors": False,
        "procfs_locator_reopen_identity_size_seals_hash_required": True,
        "source_path_reuse_after_marker": False,
        "private_key_bytes_in_logs_or_evidence": False,
        "pinning_or_procfs_failure_before_marker": True,
    }
    if pinning != expected_pinning:
        raise SemanticAdmissionError("fixed parent-procfd pinning contract drifted")
    capability = plan.get("local_openssh_consumption_capability", {})
    if (
        capability.get("required_before_final_authority_sample") is not True
        or capability.get("real_ssh_executable") != "/usr/bin/ssh"
        or capability.get("real_sshd_executable") != "/usr/sbin/sshd"
        or capability.get("real_ssh_keygen_executable") != "/usr/bin/ssh-keygen"
        or capability.get("transport") != "LOCAL_PROXYCOMMAND_PIPE_NO_SOCKET_NO_NETWORK"
        or any(value is not True for key, value in capability.items() if key not in {
            "real_ssh_executable", "real_sshd_executable", "real_ssh_keygen_executable",
            "transport", "private_bytes_recorded",
        })
        or capability.get("private_bytes_recorded") is not False
    ):
        raise SemanticAdmissionError("local OpenSSH consumption capability drifted")
    ...
    if set(plan.get("forbidden_semantics", {}).values()) != {True}:
        raise SemanticAdmissionError("fixed action plan v6 lost a prohibition")
    if plan.get("authority_boundary") != PERMISSIONS:
        raise SemanticAdmissionError("fixed action plan v6 widened authority")
    ...
    if (
        ...
        or names.get("failure_retention_relative_path") != FAILURE_RETENTION_NAME
...
def _verify_contract_action_inputs(
    ...
    stand_id, ssh_target, known_host = (
        target.get("stand_id"), target.get("ssh_target"), target.get("known_hosts_host")
    )
    ...
    if contract.get("limits") != LIMITS or contract.get("authority_boundary") != PERMISSIONS:
        raise SemanticAdmissionError("supporting contract limits or permission matrix drifted")
    ...
    evidence_root, evidence_root_identity = _safe_evidence_root(root, contract.get("evidence_root"))
    return {
        "plan": plan, "plan_path": plan_path, "pilot_contract": pilot,
        "pilot_contract_path": pilot_path, "pinned_path": pinned_path,
        ...
        "execution_paths": execution_paths, "evidence_root": evidence_root,
        "evidence_root_identity": evidence_root_identity,
        "archive_locator": archive, "sidecar_locator": sidecar,
        "bundle_root_locator": bundle_root,
    }


def verify_s17_ext_001_semantics_v8(
    *, root: pathlib.Path, resolution: dict[str, Any],
    ...
    envelopes = [(path, document) for path, document in repository_documents if
        document.get("schema_version") == "cpu-prefetch-stage17-operational-evidence-envelope/8"]
    if len(envelopes) != 1 or len(repository_documents) != 1:
        raise SemanticAdmissionError("S17-EXT-001 requires exactly one v8 envelope")
    _, envelope = envelopes[0]
    _validate_schema(root, envelope, ENVELOPE_SCHEMA_PATH, "S17-EXT-001 v8 envelope")
    if envelope["semantic_policy"] != {
        "path": policy_path.relative_to(root).as_posix(),
        "size_bytes": policy_path.stat().st_size, "sha256": policy_sha256,
    }:
    ...
    if envelope["predecessor"] != {
        ...
        "resolution_schema_sha256": resolution_schema_sha256,
        "semantic_policy_v7_sha256": policy["predecessor"]["policy_v7"]["sha256"],
        "adr_0111_sha256": policy["predecessor"]["adr_0111"]["sha256"],
        "adr_0112_sha256": policy["predecessor"]["adr_0112"]["sha256"],
    }:
        raise SemanticAdmissionError("S17-EXT-001 predecessor binding mismatch")
    expected_plan = {**policy["fixed_action_plan"],
        "schema_identity": "cpu-prefetch-stage17-read-only-preflight-fixed-action-plan/6"}
    ...
    if envelope["authorization"].get("schema_identity") != "cpu-prefetch-stage17-read-only-preflight-authorization/8":
        raise SemanticAdmissionError("authorization schema identity mismatch")
    authorization = _load_json(authorization_path)
    _validate_schema(root, authorization, AUTHORIZATION_SCHEMA_PATH, "authorization v8")
    contract_path = _binding(root, envelope["supporting_contract"], "supporting contract")
    if envelope["supporting_contract"].get("schema_identity") != "cpu-prefetch-stage17-read-only-preflight-supporting-contract/8":
        raise SemanticAdmissionError("supporting-contract schema identity mismatch")
    contract = _load_json(contract_path)
    _validate_schema(root, contract, CONTRACT_SCHEMA_PATH, "supporting contract v8")
    ...
    if authorization["fixed_action_plan"] != expected_plan:
        raise SemanticAdmissionError("authorization does not bind action plan v6")
    ...
    expected_scope = (f"STAND_ID={target['stand_id']};SSH_TARGET={target['ssh_target']};"
    ...
    if any((authorization["role_collapse_acknowledged"] is not True,
            authorization["independent_review_claimed"] is not False,
            authorization["automatic_transition"] is not False,
            authorization["retry_allowed"] is not False,
            authorization["stage18_authority"] is not False)):
    ...
    return {"authorization": authorization, "context": {
        ...
        "policy_sha256": policy_sha256, **verified,
    }}


def reverify_action_inputs(
    root: pathlib.Path, context: dict[str, Any],
    runtime_identity_paths: dict[str, str] | None,
) -> dict[str, Any]:
    policy_path, contract_path, authorization_path = (
        context["policy_path"], context["contract_path"], context["authorization_path"]
    )
    for path, digest, label in (
        (policy_path, context["policy_sha256"], "semantic policy"),
        (contract_path, context["contract_sha256"], "supporting contract"),
        (authorization_path, context["authorization_sha256"], "authorization"),
    ):
    ...
    runtime_hashes = _verify_loaded_runtime(
        policy=policy, contract=contract, runtime_identity_paths=runtime_identity_paths
    )
    for identity in contract["prospective_local_action_identities"]:
        _external_executable(identity["execution_path"], identity["source_binding"],
    ...
    fixed_names = [
        *(f"stage17-read-only-preflight-attempt-v{version}.json" for version in range(1, 7)),
        ...
        "stage17-read-only-preflight-failure-v5.json", FAILURE_NAME,
        ...
        "stage17-read-only-preflight-completion-v4.json", COMPLETION_NAME,
    ]
    fixed_names.extend(f"s17-ro-{ordinal:03d}{suffix}" for ordinal in range(1, 7)
        for suffix in (".stdout.bin", ".stderr.bin", ".receipt-v1.json", ".receipt-v2.json", ".receipt-v3.json", ".receipt-v4.json", ".receipt-v5.json"))
    if any(os.path.lexists(evidence_root / name) for name in fixed_names):
        raise SemanticAdmissionError("one-shot predecessor/current marker or output already exists")
    ...
    pre_marker_file_bindings = [{
        "locator": str(path), "size_bytes": path.stat().st_size, "sha256": digest,
    } for path, digest in (
        (policy_path, context["policy_sha256"]),
        (contract_path, context["contract_sha256"]),
        (authorization_path, context["authorization_sha256"]),
    ...
    return {
        ...
        "evidence_root": str(evidence_root), "evidence_root_identity": root_identity,
        "attempt_marker_name": ATTEMPT_NAME, "failure_name": FAILURE_NAME,
        "failure_retention_name": FAILURE_RETENTION_NAME,
        "completion_name": COMPLETION_NAME, "ssh_target": target["ssh_target"],
        "known_hosts_binding": {"locator": str(known_hosts_path),
...
def evaluate_s17_ext_001_action_readiness(
    *, root: pathlib.Path, current_state: str,
    ...
    resolution_id: str, resolution_sha256: str,
    authorization: dict[str, Any], semantic_context: dict[str, Any],
    as_of_utc: str, runtime_identity_paths: dict[str, str] | None,
) -> dict[str, Any] | None:
    ...
    if transition.get("evidence_resolutions") != [{
        "input_id": "S17-EXT-001", "resolution_id": resolution_id,
    ...
    action.update({"resolution_id": resolution_id, "resolution_sha256": resolution_sha256,
        "transition_id": transition_id, "transition_sha256": transition_sha256})
    return action
