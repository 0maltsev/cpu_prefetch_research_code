#!/usr/bin/env python3
"""Stage 17 registry with the D-124 no-predecessor-attestation successor.

This is a new, additive file.  It does not modify
`stage17_semantic_verifier_v18.py`'s bytes, so the already-accepted
`stage17-operational-evidence-admission-policy-v18.json` runtime-closure
hash for that file remains valid and unchanged.  It delegates `S17-EXT-001`
to `stage17_read_only_preflight_semantic_verifier_v15`, which accepts either
the unchanged three-blocker-receipt branch or the new
`no_predecessor_attestation` branch (ADR-0124), and every other input
unchanged to predecessor `stage17_semantic_verifier_v18`.
"""
...
import stage17_semantic_verifier_v18 as predecessor
...
def verify_policy_v19(*, root: pathlib.Path, policy: dict[str, Any],
    ...
    if errors:
        raise SemanticAdmissionError(
            f"policy v19 schema rejection: {errors[0].message}"
        )
    ...
    if not required.issubset(discovered):
        raise SemanticAdmissionError("policy v19 runtime closure is incomplete")


def verify_policy_v14(**arguments: Any) -> None:
    verify_policy_v19(**arguments)
