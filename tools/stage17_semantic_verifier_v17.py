#!/usr/bin/env python3
"""Stage 17 registry with the D-122 terminal-contract successor."""
...
import stage17_read_only_preflight_semantic_verifier_v13 as preflight
import stage17_semantic_verifier_v16 as predecessor
...
def verify_policy_v17(*, root: pathlib.Path, policy: dict[str, Any],
    ...
    if errors:
        raise SemanticAdmissionError(
            f"policy v17 schema rejection: {errors[0].message}"
        )
    ...
    if not required.issubset(discovered):
        raise SemanticAdmissionError("policy v17 runtime closure is incomplete")


def verify_policy_v14(**arguments: Any) -> None:
    verify_policy_v17(**arguments)
...
def _verify_ext001(**arguments: Any) -> dict[str, Any]:
    ...
    try:
        ...
        return preflight.verify_s17_ext_001_semantics_v13(
