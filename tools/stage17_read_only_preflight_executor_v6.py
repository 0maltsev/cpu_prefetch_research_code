...
import stage17_process_group_supervisor_v1 as process_supervisor
...
import stage17_state_journal_v5


EXECUTOR_ID = "STAGE17-READ-ONLY-PREFLIGHT-EXECUTOR-v6"
...
def runtime_identity_paths() -> dict[str, str]:
    """Return the exact loaded policy-v8 runtime closure."""

    return {
        "semantic_verifier": str(pathlib.Path(stage17_semantic_verifier_v8.__file__).resolve()),
        ...
        "state_journal": str(pathlib.Path(stage17_state_journal_v5.__file__).resolve()),
        ...
        "state_journal_v4_helper": str(pathlib.Path(stage17_state_journal_v4.__file__).resolve()),
...
def _write_failure_retention(
    *, prepared: PreparedAction, failure: OperationalFailure,
    ...
    _write_exclusive_at(
        ...
        stage17_state_journal_v5.canonical_json_bytes(document) + b"\n",
    )
...
) -> stage17_state_journal_v5.JournalValidation:
    return stage17_state_journal_v5.validate_journal(
...
def execute_once(
    *, repository_root: pathlib.Path, latest_journal: pathlib.Path,
    ...
    try:
        ...
        context = prepared.context
        issued = stage17_state_journal_v5.parse_utc(
            context["authorization"]["issued_at_utc"], "actual authority issue"
        )
        expires = stage17_state_journal_v5.parse_utc(
            context["authorization"]["expires_at_utc"], "actual authority expiry"
        )
        prospective = stage17_state_journal_v5.parse_utc(
            prospective_utc, "prospective authority UTC"
        )
        before_marker = stage17_state_journal_v5.parse_utc(
        ...
        marker = {
            "schema_version": "cpu-prefetch-stage17-read-only-preflight-attempt/6",
        ...
        try:
            prepared.attempt_marker_sha256 = _write_exclusive_at(
                ...
                stage17_state_journal_v5.canonical_json_bytes(marker) + b"\n",
            )
            marker_created = True
        ...
        try:
            for descriptor, program in zip(prepared.program_descriptors, prepared.programs, strict=True):
                ...
                def final_authority_guard() -> str:
                    ...
                    try:
                        ...
                        actual = stage17_state_journal_v5.parse_utc(
                ...
                try:
                    ...
                    receipt_hashes.append(_write_exclusive_at(
                        ...
                        stage17_state_journal_v5.canonical_json_bytes(receipt) + b"\n",
                    ))
                except BaseException as exception:
                    raise OperationalFailure(
            ...
            try:
                _write_exclusive_at(
                    ...
                    stage17_state_journal_v5.canonical_json_bytes(completion) + b"\n",
                )
            except BaseException as exception:
                raise OperationalFailure(
...
def main() -> int:
    ...
    except (ActionExecutionError, stage17_state_journal_v5.JournalError, OSError, ValueError) as exception:
        print(f"stage17-read-only-preflight-v6: FAIL: {exception}", file=sys.stderr)
        return 1
    print("stage17-read-only-preflight-v6: PASS fixed=6 retry=0 Stage18=false")
    return 0
