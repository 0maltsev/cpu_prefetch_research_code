...
The Q10/Q11 protocol and statistical authority record resolves the prior
simultaneous-blocker ambiguity through an exhaustive blocker set and
non-priority `BLOCKED_MULTIPLE` summary. This representation-only amendment
does not change raw observations, gate applicability, estimands, or the
complete-block replacement rule.

The Q17B2-PROT protocol and statistical authority record resolves the
first-pilot warm-up cycle. It permits exactly the existing five-second floor
only for explicitly marked non-confirmatory pre-freeze calibration/pilot runs,
then requires a complete lineage-bound pilot freeze before later use. It does
not supply a duration beyond that floor, a platform value, or execution
authority.
...
| Warm-up, reset, start, and carryover | `READY_FOR_IMPLEMENTATION` | Dedicated namespace, exact five-second `PRE_FREEZE_BOOTSTRAP`, complete run-level extrema/lineage freeze, upward tick rounding, exact logical origin, preserved mappings, start barrier, warm-start interpretation, and recovery control are fixed. Numerical post-pilot output remains unavailable before valid pilot evidence. |
