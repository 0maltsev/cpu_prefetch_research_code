...
| Stage 17 operational authorization | `SOFTWARE_READY_FOR_FRESH_RELEASE_AND_READ_ONLY_PREFLIGHT; EXTERNAL_INPUTS_REQUIRED` | ADR-0117/policy v12 supplies the durable pilot, Q15/Q16, exit, and Phase 18 design. ADR-0119 binds policy v14, preflight policy v11, journal v12 and executor v9 after the stopped executor-v8 transaction. ADR-0120 adds policy v15, journal v13, controller v5, Q15 controller v3 and CLI v7, closing the post-T2 policy mismatch without changing records or action semantics. The persisted external-journal regression passes 4 positive/5 negative cases: the exact T1 journal independently validates under the preflight and successor runtimes, all six programs prepare, controller v4 remains rejected, and marker/transport counts remain zero. A fresh clean release and fresh S17-EXT-001 are required before the real read-only preflight; Q15/Q16/pilot still require their ordered real evidence and separate authority. Checked-in state remains 0 resolutions, 0 transitions, ten missing, `action_ready=false`, `pilot_ready=false`, `stand=NOT_ACCESSED`. |
