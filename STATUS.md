...
| Stage 2/16 implementation-decision record | `COMPLETE`; D-116 accepted | ADR-0001 through ADR-0112 remain immutable decisions. ADR-0113/policy v9 is `REJECTED_FAIL_OPEN_PREDECESSOR`; ADR-0114/policy v10 and ADR-0115/policy v11 are immutable incomplete predecessors. ADR-0116 accepts the pre-freeze warm-up bootstrap in protocol pre.3; ADR-0117/policy v12 is the current repository-local durable-pilot/handoff boundary and issues no authority. D-099..D-108 decision/evidence bytes remain hash-preserved. |
...
| Stage 17 operational authorization | `LOCAL_ENGINEERING_CLOSED; PREPARED_EXTERNAL_INPUTS_REQUIRED` | ADR-0117/policy v12 hash-binds the pre.3 runtime/schema closure, verifies a full clean bundle before EXT006, derives synthetic/production class from verified bytes, and requires the observed EXT002/003 runner to equal the release member. Q15-R/Q15-W is one supervised transaction with live prestate, real probes and restore/readback; Q16 and pilot require complete frozen matrices. The durable pilot session requires all 180 cells and frozen repetitions, gives every run an append-only marker/deadline/terminal record, and streams independently decoded raw evidence under plan-derived bounds. Controller v4 validates its action-specific typed output registry only after quiescence. Stage 17 exit is computed from exact typed pilot evidence; Phase 18 has independent trust and chronology. Fresh dev, ASan/UBSan, and TSan matrices pass 339/339 each; focused clang-tidy, formatting, dual-disassembler gates, and the clean public-CLI workflow pass. The latter reports 12 positive/12 negative cases but creates only temporary synthetic evidence. Checked-in state remains 0 resolutions, 0 transitions, ten missing, `action_ready=false`, `pilot_ready=false`, `stand=NOT_ACCESSED`. |
...
Phase 18 access locally. The bundle-root handoff
