...
import stage17_fixed_action_executor_v3 as action_executor
...
import stage17_phase_controller_v3 as phase_controller
import stage17_process_group_supervisor_v2 as supervisor


WAITING_NAME = "stage17-q15-session-waiting-v1.json"
Q15_R_RESULT_NAME = "stage17-q15-r-result-v3.json"
Q15_W_RESULT_NAME = "stage17-q15-w-result-v3.json"
    ...
    *, q15_r: phase_controller.PreparedAction,
    ...
    try:
        ...
        request_snapshot = action_executor.predecessor._pin_generated(
            q15_r.request_bytes, "Q15_R_REQUEST_V3"
        )
        ...
        context_snapshot = action_executor.predecessor._pin_generated(
            _canonical(context_document), "Q15_R_CONTEXT_V3"
        )
        ...
        action_executor.predecessor._write_exclusive(
            directory_fd, "stage17-q15_r-attempt-v3.json", _canonical(attempt)
        )
        marker_created = True
        ...
        q15_r_attempt_bytes = action_executor.predecessor._read_at(
            directory_fd, "stage17-q15_r-attempt-v3.json", 1024 * 1024
        )
        if (
        ...
        action_executor.predecessor._write_exclusive(
            directory_fd, "stage17-q15_w-attempt-v3.json",
        ...
        completion_name = "stage17-q15_w-completion-v3.json"
        action_executor.predecessor._write_exclusive(
...
def run_start(arguments: argparse.Namespace) -> int:
    ...
    listener.listen(1)
    expiry = phase_controller.predecessor._parse_utc(
        prepared.authorization["expires_at_utc"]
    )
    remaining = (expiry - phase_controller.predecessor._parse_utc(
        phase_controller.predecessor._now()
    )).total_seconds()
    if remaining <= 0:
        listener.close()
