#!/usr/bin/env python3
"""Exact-byte, closed-role Stage 17 operational evidence admission v3.
...
import stage17_pilot_candidate_artifact_v3 as release_verifier
import stage17_fixed_action_executor_v2 as snapshot_factory
import stage17_openssh_parent_snapshot_v1 as snapshot_broker


MANIFEST_SCHEMA = "config/schemas/stage17-operational-input-manifest-v3.schema.json"
ARTIFACT_SCHEMA = "config/schemas/stage17-operational-artifact-v3.schema.json"
TYPED_SCHEMA = "config/schemas/stage17-operational-typed-record-v3.schema.json"
AUTH_SCHEMA = "config/schemas/stage17-phase-action-authorization-v3.schema.json"
REQUEST_SCHEMA = "config/schemas/stage17-fixed-action-request-v3.schema.json"
ATTEMPT_SCHEMA = "config/schemas/stage17-phase-action-attempt-v3.schema.json"
RESULT_SCHEMA = "config/schemas/stage17-phase-action-result-v3.schema.json"
COMPLETION_SCHEMA = "config/schemas/stage17-phase-action-completion-v3.schema.json"
SUPPORTED_ACTIONS = (
...
ROLE_SCHEMA: dict[str, str | None] = {
    ...
    "RUNTIME_RELEASE_PROVENANCE": "config/schemas/stage17-runtime-release-provenance-v3.schema.json",
    ...
    "PLATFORM_MANIFEST": "protocol/2.0.0-pre.2/handoff/schemas/platform.schema.json",
    ...
    **{role: COMPLETION_SCHEMA for role in (
    ...
    "PILOT_PLAN_V3": "config/schemas/stage17-pilot-plan-v3.schema.json",
    ...
    "PHASE_INTEGRITY": "config/schemas/phase-integrity-report-v1.schema.json",
    ...
    "STAGE17_PILOT_HARDWARE_STATE": "config/schemas/stage17-pilot-hardware-state-v1.schema.json",
    "STAGE17_BLINDED_PILOT_RUN": "config/schemas/stage17-run-output-v3.schema.json",
    "SEALED_PILOT_ARTIFACT_MANIFEST": "config/schemas/stage17-sealed-pilot-artifact-manifest-v3.schema.json",
}
    ...
    if validator is None:
        ...
        if relative == MANIFEST_SCHEMA:
            ...
            registry = registry.with_resource(
                "stage17-operational-artifact-v3.schema.json", resource
            ).with_resource(
                "urn:cpu-prefetch:stage17:operational-input-manifest:stage17-operational-artifact-v3.schema.json",
                resource,
            )
    ...
    for reference in manifest["artifacts"]:
        ...
        if expected_schema is None:
            if role in LOGICAL_ENVELOPE_ROLES:
                if (reference["schema_identity"] != "2.0.0-pre.2"
...
def _verify_extracted_release(
    ...
    try:
        context = release_verifier.verify_extracted_release_receipt_v3(
...
EXPECTED_ROLES: dict[str, collections.Counter[str]] = {
    ...
    "S17-EXT-008": collections.Counter({"PILOT_PLAN_V3": 1}),
...
def verify_manifest_v3(
    *, repository_root: pathlib.Path, manifest_path: pathlib.Path,
    ...
    _validate(root, manifest, MANIFEST_SCHEMA, "operational manifest v3",
    ...
    if expected_input_id == "S17-EXT-002":
        ...
        context.update({"q15_r": family, "qualification_evidence": evidence})
        ...
        plan_artifact = _one(artifacts, "PILOT_PLAN_V3")
        assert plan_artifact.document
        _validate_pilot_plan(
            plan_artifact.document, stand_id=manifest["stand_id"],
            synthetic=allow_synthetic, admitted_resolutions=admitted_resolutions,
            pinned=pinned_repository_bytes,
        )
