...
| Stage 2/16 implementation-decision record | `COMPLETE`; pilot-governance successor recorded | ADR-0001 through ADR-0086, ADR-0093 through ADR-0097, ADR-0099, and ADR-0100 through ADR-0110 are accepted. ADR-0105 implements append-only state persistence; ADR-0106 adds the immutable default-deny semantic predecessor; ADR-0107 adds fixed action semantics and transition/one-shot gating; ADR-0108 and ADR-0109 remain immutable actual-clock/durable-one-shot and sealed-input predecessors. ADR-0110 adds the policy-v6 parent-procfd consumption successor after real OpenSSH disproved inherited `/proc/self/fd/N`, without changing ADR-0104's graph or any predecessor identity. D-099..D-108 decision/evidence bytes are hash-preserved; D-105..D-108 remain proposed/unaccepted. |
...
| Stage 17 operational authorization | `PREPARED_EXTERNAL_INPUTS_REQUIRED` | ADR-0104 provides the only successor graph; ADR-0105 preserves the v1 successor/checklist and computes state from the append-only journal; ADR-0106 through ADR-0109 remain immutable predecessors. ADR-0110 policy v6/plan v4 bind verifier v6, executor v4, journal runtime v3, unchanged collector v2, the parent-procfd broker, and every record/schema/helper byte. Real local OpenSSH characterizes inherited self-procfd failure, then proves strict host-key and public-key consumption from sealed `/proc/<procfs-visible-parent>/fd/N` snapshots with no inherited credential descriptors, socket, DNS, network, or stand. Eight focused positive and twenty focused negative 17A.5 cases pass alongside two runtime positives, seven 17A.4 positives, five 17A.4 negatives, and all 79 predecessor negatives. `S17-EXT-006` retains exact-byte verification; the other eight inputs fail closed. The checked-in journal has 0 resolutions and 0 transitions, all ten inputs are missing, `action_ready=false`, `pilot_ready=false`, and `stand=NOT_ACCESSED`. No stand, preflight, phase, measurement, or Stage 18 authority exists. |
...
v6 the current production admission after real `/usr/bin/ssh` characterization
...
Journal runtime v3 and the broker are explicitly policy-hash-bound while all
predecessors remain immutable.
