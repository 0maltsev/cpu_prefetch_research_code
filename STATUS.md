...
| Stage 2/16 implementation-decision record | `COMPLETE`; D-116 accepted | ADR-0001 through ADR-0112 remain immutable decisions. ADR-0113/policy v9 is `REJECTED_FAIL_OPEN_PREDECESSOR`; ADR-0114/policy v10 and ADR-0115/policy v11 are immutable incomplete predecessors. ADR-0116 accepts the pre-freeze warm-up bootstrap in protocol pre.3; ADR-0117/policy v12 remains the durable-pilot predecessor. ADR-0118/policy v13 and preflight policy v10 are preserved runtime predecessors. ADR-0119/policy v14 fixes only the missing canonical serializer in the read-only-preflight closure and issues no authority. D-099..D-108 decision/evidence bytes remain hash-preserved. |
...
| Stage 17 operational authorization | `READ_ONLY_PREFLIGHT_SOFTWARE_READY; FRESH_RELEASE_AND_AUTHORITY_REQUIRED; POST_PREFLIGHT_CONTROLLER_BLOCKED` | ADR-0117/policy v12 supplies the durable pilot, Q15/Q16, exit, and Phase 18 design. After the external-root and nested-bundle corrections, the first T1 execution exposed executor v8's missing canonical-serializer import before marker/transport. ADR-0119 binds policy v14, preflight policy v11, CLI v6, journal v12, verifier v11/v14, and executor v9. Its persisted external-journal regression passes 3 positive/4 negative cases through full preparation of six programs with zero marker/transport; v8 remains immutable and characterized. Every prior partial transaction is retained without retry. Controller v4 still cannot consume the successor journal after T2 and remains a later blocker before Q15. A fresh clean release and fresh S17-EXT-001 are required before exactly one read-only preflight; no Q15/Q16/pilot action is eligible. Checked-in state remains 0 resolutions, 0 transitions, ten missing, `action_ready=false`, `pilot_ready=false`, `stand=NOT_ACCESSED`. |
